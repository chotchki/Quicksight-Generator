"""Browser tests: AR cross-sheet drill-downs navigate to the target sheet.

L.11.4 — parametrized over every cross-sheet, left-click `Drill` the
tree declares (via `enumerate_cross_sheet_left_click_drills(ar_app)`).
Adding a new drill in the tree auto-extends test coverage with no
test edits.
"""
...
def _ar_drill_specs():
    """Build the parametrize list at collection time.

    Loads the same `cfg` the session-scoped `ar_app` fixture uses, then
...
@pytest.mark.parametrize(
    "src_sheet_name,src_visual_title,tgt_sheet_name", _ar_drill_specs(),
)
def test_drill_navigates_to_target_sheet(
    ...
    the dashboard to the target sheet. Every cross-sheet, left-click
    Drill the tree declares is a parametrized case."""
