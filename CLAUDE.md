...
- **Docs prose: bullet lists of 4+ items, not slash-separated.** "X / Y / Z / W / V" gets unreadable fast — readers scan past it. Convert any in-prose enumeration of 4+ items (especially with parenthetical descriptions) to a `-` Markdown bullet list under the sentence that introduces it. Slash-separated naming is fine for **section / page titles** that group sister concepts ("Tree — App / Analysis / Dashboard / Sheet") and for **2-3 item lists**. The bar to bullet is "would scanning this be faster as a column?" — if yes, bullet it.
