...
`src/recon_gen/common/l2/auto_scenario.py` (plant overlays). The
CLI `data apply` calls `build_full_seed_sql(cfg, instance)` in
`src/recon_gen/cli/_helpers.py`, which composes the four-stage
pipeline:
...
`src/recon_gen/common/l2/seed.py`; per-kind defaults at
`_RAIL_KIND_PARAMS`.
