...
- A ``diagram(family, **kwargs)`` macro that dispatches to the render
  functions in ``recon_gen.common.handbook.diagrams``.
...
# ``src/recon_gen/_l2_fixtures/`` (see pyproject ``package_data``).
# Same path resolves in both dev (``Path(__file__).parent`` =
# ``<repo>/src/recon_gen/``) and installed mode
# (``<site-packages>/recon_gen/``).
