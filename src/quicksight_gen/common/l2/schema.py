...
def _render_limit_breach_cases(instance: L2Instance, *, p: str) -> str:
    ...
    Returns a multi-line SQL CASE expression. If the instance has no
    LimitSchedules, returns ``NULL`` so the view emits valid SQL but
    surfaces no rows.
    """
    ...
    for ls in instance.limit_schedules:
        ...
        branches.append(
            f"WHEN child_db.account_parent_role = '{ls.parent_role}' "
            f"AND outbound.transfer_type = '{ls.transfer_type}' "
            f"THEN {ls.cap}"
        )
    branches_sql = "\n        ".join(branches)
    return f"CASE\n        {branches_sql}\n        ELSE NULL\n    END"
...
WITH outbound AS (
    ...
    GROUP BY tx.account_id, DATE_TRUNC('day', tx.posting), tx.transfer_type
)
SELECT
    outbound.account_id,
    child_db.account_name,
    child_db.account_role,
    child_db.account_parent_role,
    outbound.business_day,
    outbound.transfer_type,
    outbound.outbound_total,
    {limit_cases} AS cap
FROM outbound
JOIN {p}_current_daily_balances child_db
  ON child_db.account_id = outbound.account_id
 AND child_db.business_day_start <= outbound.business_day
 AND child_db.business_day_end > outbound.business_day
WHERE child_db.account_parent_role IS NOT NULL
  AND ({limit_cases}) IS NOT NULL
  AND outbound.outbound_total > ({limit_cases});
"""
