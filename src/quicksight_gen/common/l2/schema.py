"""Prefix-aware SQL DDL emission for an ``L2Instance`` (M.1.4).

Emits idempotent (drop-then-create) DDL for the L1 base tables that any
L2 instance produces, prefixed per the instance's ``InstancePrefix`` per
the SPEC's storage-isolation rule (F10):

  ``<prefix>transactions``    — L1 ``Transaction`` denormalized with
                                Account + Transfer fields per the
                                Implementation Entities section.
  ``<prefix>daily_balances``  — L1 ``StoredBalance`` denormalized with
                                Account fields.

Plus B-tree indexes for the dashboard's hot-path queries.

Current* views (CurrentTransaction / CurrentStoredBalance) ship in
M.1.5, not here — they're derivatives the dashboard SQL targets, not
base tables the integrator's ETL writes to.
