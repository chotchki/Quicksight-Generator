...
Output:
  ...
  - [ ] If the demo deployment is done, an additional sheet at the front of the dashboard should be added explaining the scenario and what the other sheets are for.
	...
	- [ ] Side-by-side tables with mutual filtering. Two related tables placed beside each other, where clicking a row in one filters the other (and vice versa), is a very powerful match-troubleshooting pattern — it lets users pivot between "what's on this side" and "what matches it on the other side" without changing tabs. Leverage this pattern wherever reconciliation or matching is the user's job: the Payment Reconciliation tab does this for internal payments vs. external transactions, and it should be the default reach for any future matching workflow.
