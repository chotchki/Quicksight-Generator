Goal: 
	- [x] To create one or more AWS Quicksight analyses with a series of tabs to help understand a financial application.

Background:
	- [x] This financial application uses a SQL datasource. For production use, a pre-existing datasource ARN is provided. For demo mode, the project creates a QuickSight data source definition from the demo database connection URL.

	- [x] This SQL datasource will be accessed using quicksight custom sql data sets that this project will need to create against the data source. Dummy SQL queries are fine for the moment.
	...
	- [x] This analysis will want to make queries to support each of those steps, provide controls to filter and easy ways to check if a settlement didn’t happen or payments were returned.
	- [x] The financial application needs to ensure the payments match to other external systems since only payments would leave the system. There will be successful matches, not yet matched and late matching which will be exceptions. (Consolidated into a single analysis as the Payment Reconciliation tab rather than a separate analysis.)
...
Key Domain Models to Understand:
	...
	- The payments are aggregated in multiple external systems as a series of transactions. Those transactions have unique identifiers and will each contain a certain amount of payments. All external system data is accessed through the single configured datasource.
	- A match is only valid when the aggregation in the external system AND the payments equal each other exactly. No partial matches — totals must be equal.
...
Output:
	- [x] Create something that creates the json that could be imported using the AWS cli to create the described analysis. The thing could be python, terraform, some combination of it.

	- [x] The customer of these aws quicksight reports doesn’t know what they want so an ability to mutate this project either via you (Claude) OR some level of structured code that generates the quick sight reports will be VERY helpful.
	
	- [x] Visualizing what matches internally vs externally may be a great use of a sankey diagram. (Replaced with dual mutually-filterable tables — click an external transaction to see its payments, click a payment to see its transaction.)

	- [x] The customer likely doesn’t understand this spec, the quicksight dashboards should include easy to understand explations to help someone understand what each analysis, sheet, visualization does.

	- [X] Build a consistent theme that looks nice, color theme blues and greys but must be easily read. Apply the theme and reasonable styling to everything produced.
