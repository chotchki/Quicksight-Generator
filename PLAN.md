...
- [x] **L.1 — Tree primitives in `common/`.** Shipped as the `common/tree/` package (split when it crossed the 600-line trigger in commit `23d5a0b`). Acceptance met: primitives shipped with full unit coverage (699 tests); `to_aws_json()` round-trip test over the kitchen-sink app green; the L.0 spike's Account Network port re-implemented against the full primitive set emits byte-identical JSON (L.1.15).
  ...
  - [ ] L.1.22 — Factory splits for filter mutual-exclusion (Bucket B): eliminate the 6 `__post_init__` mutual-exclusion raises by splitting wrappers with mutually-exclusive modes into per-mode factory methods. Same pattern `Dim.date()` / `Measure.sum()` already use. Sub-steps:
    - L.1.22.1 — `CategoryFilter.with_values(dataset, column, values, ...)` and `CategoryFilter.with_parameter(dataset, column, parameter, ...)`. Standalone `CategoryFilter(...)` constructor goes private. Eliminates the two "neither/both" raises (`filters.py:135` + `:140`).
    - L.1.22.2 — `NumericRangeFilter` — design choice: either four factories (`with_static_min`, `with_static_max`, `with_parameter_min`, `with_parameter_max` — composable via chained calls) OR a typed `Bound = StaticBound(value) | ParameterBound(param)` discriminated union with a single `bounded(min: Bound | None = None, max: Bound | None = None)` constructor. Pick the cleaner option during sub-step. Eliminates the two "both min/max set" raises (`filters.py:210` + `:215`).
    - L.1.22.3 — `LinkedValues.from_column(col)` and `LinkedValues.from_string(dataset, column_name)`. The `Column` form derives the dataset; the string form requires it explicitly. Eliminates the two `__post_init__` raises (`controls.py:106` + `:113`).
    - L.1.22.4 — Migration: same files as L.1.21. Mechanical.
    - L.1.22.5 — Tests: drop the 6 rejection tests for the eliminated raises; update L.1.18 audit table.
    - L.1.22.6 — Run pytest + pyright; CLI JSON byte-identical pre/post.

    Net across L.1.21 + L.1.22: 10 runtime validators eliminated by construction; tree's runtime-raise inventory drops from 36 → 26. Sequencing: do BOTH before L.3 starts so the AR port writes against the new prescriptive API from the start (saves ~80% of an L.3 follow-up rework).
