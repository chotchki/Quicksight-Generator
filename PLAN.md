...
- [ ] **X.2.l.4 — Filter-widget parity (fancy date picker / multi-select / slider).** App 2 currently renders native `<input type="date">` / `<select multiple>` / `<input type="range">` — functional but visibly clunky next to QuickSight's polished widgets, and `<select multiple>` is essentially un-stylable into a chip/token UI. Fix it the way App 2 already CDN-loads d3 + htmx + the wasm-graphviz: pull three small zero-dep widget libs from CDN. Picks: **Tom Select** (multi-select with chips/search — the one that *matters*, since native multi-select can't be styled into the QS shape), **Flatpickr** (date-range popover + presets), **noUiSlider** (draggable thumb + value bubble; two-handle for min/max). Scope is contained: App 2 already auto-derives filter widgets from the tree's `ParameterControl` nodes (`X.2.g.2.f/g`), so the work is "in the renderer's render-a-filter-control branch, emit the fancy markup + a one-line init per filter-spec kind" + a Tailwind-token CSS override sheet so the libs pick up the L2 theme (the X.2.l theme integration applies — each lib exposes CSS-custom-property hooks). Alternative considered + rejected: hand-rolled vanilla-JS micro-widgets (more surface area to own — keyboard nav / mobile touch / a11y edge cases — for strictly less polish).
  - [ ] **X.2.l.4.a** — wire the three libs from CDN (`<script>`/`<link>` in the page shell, same place as d3/htmx) + the init shim that re-runs on HTMX swaps.
  - [ ] **X.2.l.4.b** — renderer emits the fancy markup per filter-spec kind: `date_range` → Flatpickr, `multi_select` (dropdown / checkbox-list controls) → Tom Select, `slider` / number-range → noUiSlider; single-select / category stays a styled `<select>` (Tom Select single-mode if it earns it).
  - [ ] **X.2.l.4.c** — Tailwind-token CSS override sheet so the three libs render in the L2 theme (`--color-accent` / `--color-surface` / etc. — the X.2.l semantic tokens). Carries the a11y rule (color not the only indicator) into the widget chrome.
  - [ ] **X.2.l.4.d** — Playwright round-trip per widget (open, pick, fetch refires with the right `param_*` query) + extend the X.2.h browser e2e.
...
- [ ] **X.2.k — Release v8.x.0 / v9.0.0.** RELEASE_NOTES, README update reflecting the 4 apps now ship in two dialects, CLAUDE.md updates for the new `serve` CLI group + the architecture, tag + push. (Gated on X.2.p — don't ship a pip-installable App 2 that needs internet at runtime.)
