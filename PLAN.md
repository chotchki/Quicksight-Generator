...
- [x] **Y.2.a — Money Trail dataset: `pInvMoneyTrailRoot` + `pInvMoneyTrailMaxHops` + `pInvMoneyTrailMinAmount`.** Three params on one dataset.
    ...
    - [ ] **Y.2.a.7 — Aurora deploy + verify (operator).** Deferred to operator deploy; mechanism is proven by Y.1.h. Expected: dropdown auto-selects on first paint via the unfiltered companion; the bridge fires on commit; main dataset SQL substitutes the chosen root + slider defaults; Sankey + table populate at the DB. Initial paint of Sankey + table is empty (sentinel default `__no_chain_selected__` matches no row) — accepted per the spike-time finding.
- [x] **Y.2.b — Account Network dataset: `pInvANetworkAnchor` + `pInvANetworkMinAmount`.** Two params on one dataset.
    ...
    - [ ] **Y.2.b.8 — Aurora deploy + verify (operator).** Deferred. Initial paint: Sankeys + table empty (sentinel anchor matches no row). Anchor dropdown auto-populates from K.4.8k companion, lands on first available; analyst commits → bridge fires → main dataset SQL substitutes the chosen anchor + min-amount default → all three visuals populate; directional FGs partition into inbound/outbound at QS engine. Same UX cost as Y.2.a.
