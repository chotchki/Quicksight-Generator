...
- [ ] **Y.2.gate.c — Implement the runner.** Substrate: pytest + `quicksight_gen/_dev/runner.py` (~400 LOC budget) + `./run_tests.sh` bash shim (locked by `Y.2.gate.b.0` spike). Each sub-task carries the module / function / hook ownership.
    ...
    - [ ] **Y.2.gate.c.14 — Dispatch-table cross-check unit test (per `Y.2.gate.b.11`).** Asserts a small canonical sample of `(layer, variant)` cells matches between `quicksight_gen/_dev/runner.py` (runtime authority) and `docs/audits/y_2_gate_test_layer_chain_audit.md` §3 (documented mirror). Sample picks the high-signal cells (layer 3a × Oracle = ✅, layer 3c × Oracle = ❌, etc.); not a full markdown parser. Catches "I edited one and forgot the other" drift loudly.
