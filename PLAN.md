...
**Y.2.a–e + app2.cde — DONE** (shipped v8.8.0a6 / a17 / a19 / a20; full detail in `RELEASE_NOTES.md` + `PLAN_ARCHIVE.md` + git history):

- **Y.2.a–e** — dataset-SQL pushdown applied to every sheet with pushdownable filters: Money Trail (`pInvMoneyTrailRoot/MaxHops/MinAmount` + a roots-companion dataset for the chain-root dropdown), Account Network (`pInvANetworkAnchor/MinAmount`), L2FT Rails (`pL2ftRail/Status/Bundle` — multi-valued `IN (<<$p>>)`), L2FT Chains (`pL2ftChainsChain/Completion`), L2FT Transfer Templates (`pL2ftTtTemplate/Completion`, bridged to BOTH the tt-instances Table + tt-legs Sankey). **The pattern Y.2.g/h follow:** wrap the projection in a subquery so CASE-aliased columns are visible to the outer WHERE; `MappedDataSetParameters` bridge from the analysis param (bridges don't fire on first load → opening view = the full declared value set / `StaticValues` default); drop the analysis-level `CategoryFilter` FilterGroup; `_populate_pushdown_dropdown` wires the param + MULTI_SELECT `StaticValues` dropdown; `tests/e2e/test_dataset_sql_smoke.py` (the Y.2.b-hotfix verifier, now pytest-collected) catches deployed-SQL exceptions. Metadata cascades (`<<$pKey>>`/`<<$pValues>>`) stay on the existing OR-cascade WHERE (per the Y.1.p verdict — works under manual interaction; URL load returns the unfiltered universe).
- **Y.2.app2.cde** — App2 SQL-executor parity: `apply_dataset_param_defaults` (substitute a QS dataset param's `StaticValues` default when the URL omits it → a fresh App2 page matches QS's initial-load render); `expand_multivalued_dataset_params` (2+ URL values for a `MULTI_VALUED` param → `:param_pX_n` IN-bind expansion, never string-spliced); L2FT is the 4th app on the generic tree-fetcher path with its MULTI_SELECT dropdowns auto-rendered into the App2 filter form (`make_filter_specs_for_sheet` tree-walk → `ParameterMultiSelectSpec`); `tests/e2e/test_html2_l2ft.py` browser e2e.

**Known pre-existing flakes** (surfaced during the Y.2.a–e browser triage; not Y.2-blocking, open follow-ups): `test_audit_dashboard_agreement[postgres-supersession]` (the supersession 3-way-agreement flake); `test_l1_filters::test_check_type_dropdown_exposes_options` (`wait_for_selector` timeout — intermittent QS-render-timing). Harden the waits / `xfail`-with-reason if persistent.

- [x] **Y.2.f — L1 universal date range pushdown — DONE (2026-05-10) via Option B: dual-SQL.** Same `app2_date_filter` dual-SQL pattern as Y.2.h, applied to all 8 L1 datasets that the analysis-level `TimeRangeFilter` family scopes (drift, ledger_drift, drift_timeline ×2, overdraft, limit_breach, todays_exceptions, transactions). Each builder converts its SQL to a `sql_template` with a `{date_filter}` slot; `qs_sql = template.format(date_filter="")` (zero QS behavior change — the analysis-level FG continues to do the date filtering); `app2_sql = template.format(date_filter=app2_date_filter("<col>", cfg.dialect))` so when L1 lands on App2 the date binds reach the SQL automatically. Ducks the Y.1.k `RollingDate`-vs-`StaticValues` wart entirely — QS never sees a dataset DateTime param, so there's no opening-view shift. Stuck-pending / stuck-unbundled / supersession / daily-statement intentionally untouched (current-state matviews; date filter would diverge from the audit PDF, breaking U.8.b's three-way agreement). Drift/ledger_drift timelines insert the `{date_filter}` slot BEFORE the `GROUP BY` so the AND-clause stays in the WHERE.
    - **Y.2.f.2 — moved to X.2.j.dateparity (2026-05-10).** Day-inclusivity verification belongs in the 4-way agreement contract.
- [x] **Y.2.g — L1 per-sheet *categorical* filter controls — DONE (2026-05-10).** Operator manual-verified the deployed sasquatch_pr QuickSight L1 dashboard (noticeably faster — the headline win). Sub-bullets g.0–g.11 ticked below. Convert `apps/l1_dashboard/app.py::_wire_per_sheet_category_filters`'s ~22 per-sheet dropdowns from `CategoryFilter.with_values(values=[], "FILTER_ALL_VALUES")` (the X.1.g cold-fetch footgun — these lazy-fetch the column's distinct values from QS's `tenK-sample-values-V2` endpoint, which 404s on cold per-CI-run dashboards) to dataset-SQL pushdown. Converting fixes the footgun *and* the pushdown together. Plus Daily Statement's 2 account FilterGroups (already analysis-`StringParam`-bound via `CategoryFilter.with_parameter`, but not yet pushed down) and its date pickers (those are Y.2.f / date territory — leave). **The ~22 split into two sub-patterns:**
    - **Enum columns (~13)** — `status` (Transactions), `transfer_type` (Limit Breach / Pending / Unbundled / Today's Exceptions / Transactions), `rail_name` (Pending / Unbundled), `account_role` (Drift / Drift Timelines / Overdraft), `supersedes` (Supersession Audit → a `SupersedeReason` enum), `check_type` (Today's Exceptions → the 6 L1-invariant names). Clean Y.2.c/d/e pattern: `MULTI_VALUED` dataset param, `StaticValues` default = the L2-declared set (`rail_name`/`transfer_type`/`account_role` walk the L2 instance) or the fixed enum (`status`/`supersedes`/`check_type`), MULTI_SELECT `StaticValues` dropdown, `<col> IN (<<$pX>>)` in the dataset SQL (sentinel `IN ('__no_match__')` fallback), `MappedDataSetParameters` bridge, drop the FilterGroup. Drift / Drift Timelines are the `cross_dataset="ALL_DATASETS"` case (one dropdown narrows both the leaf-drift + ledger-drift datasets / both timeline datasets — share column names) — bridge to BOTH datasets (like Y.2.e's TT sheet).
    - **Data-value columns (~9)** — `account_id` (Drift / Overdraft / Limit Breach / Pending / Unbundled / Today's Exceptions / Transactions / Daily Statement ×2) and `transfer_id` (Transactions). No static enum → can't be `StaticValues`. **LOCKED (user, 2026-05-10): shared companion datasets** — one new `<prefix>_l1_accounts` matview-or-CTE-backed companion dataset (`SELECT DISTINCT account_id, account_name FROM <prefix>_transactions`) that every `account_id` dropdown's `LinkedValues.from_column(...)` points at; one `<prefix>_l1_tx_ids` companion (`SELECT DISTINCT transfer_id FROM <prefix>_transactions`) for the Transactions `transfer_id` dropdown. Same shape as Investigation's `DS_INV_ANETWORK_ACCOUNTS` (K.4.8k) + Y.2.a's roots-companion. The dropdown's analysis `StringParam` (multi_valued — `account_id IN (<<$pAccount>>)`) bridges to a `<<$pAccount>>` dataset param in each filtered dataset's SQL (Y.2.app2.cde's multi-valued IN-bind expansion handles the App2 side). Daily Statement's 2 `account_id` filters are already analysis-`StringParam`-bound — just add the SQL pushdown + bridge + repoint the dropdown's source at the new companion.
    - [x] **Y.2.g.0 — Foundation.** Two helpers in `apps/l1_dashboard/app.py`: `_populate_pushdown_enum_dropdown` (mirrors L2FT's `_populate_pushdown_dropdown` — `StaticValues` for bounded enums) + `_populate_pushdown_value_dropdown` (`LinkedValues` from a companion + the `('__l1_all__' IN (<<$p>>) OR col IN (<<$p>>))` sentinel-OR data-value guard). Enum-value helpers in `datasets.py`: `l1_rail_values` / `l1_transfer_type_values` (rails ∪ limit_schedules) / `l1_account_role_values` (accounts ∪ account_templates) / `l1_supersede_reason_values` (3 v1 reasons) / `l1_check_type_values` (7 matview discriminators). **3** companion datasets, not 2: `build_l1_accounts_dataset` (DISTINCT account_id), `build_l1_tx_ids_dataset` (DISTINCT transfer_id), `build_l1_tx_facets_dataset` (DISTINCT status, origin — `Origin` turned out to be `str` not a Literal, and `status` is open-set per X.1.i, so both need a companion not StaticValues). `_mv_dataset_param` / `_sv_dataset_param` / `_data_value_clause` helpers; `PUSHDOWN_NO_MATCH_SENTINEL` for the empty-L2-enum fallback + `L1_ALL_SENTINEL` for the data-value guard. Unit-tested in `tests/json/test_l1_dashboard.py::test_y2g_*`; `tests/e2e/test_dataset_sql_smoke.py` picks up the new SQLs.
    - [x] **Y.2.g.1 — Supersession Audit** — `supersedes` enum pushdown (`pL1SupersedeReason`); SQL keeps `OR supersedes IS NULL` so the entry-1 (original) rows of an audit trail stay visible.
    - [x] **Y.2.g.2 — Limit Breach** — `transfer_type` enum (`pL1LimitBreachType`) + `account_id` data-value (`pL1LimitBreachAccount`).
    - [x] **Y.2.g.3 — Drift + Drift Timelines** — ALL_DATASETS case: `pL1DriftAccount` / `pL1DriftRole` bridged to BOTH `<prefix>_drift` + `<prefix>_ledger_drift`; `pL1DriftTlRole` bridged to BOTH timeline datasets (same-name dataset params on each, like Y.2.e's TT sheet).
    - [x] **Y.2.g.4 — Overdraft** — `pL1OverdraftAccount` (data-value) + `pL1OverdraftRole` (enum).
    - [x] **Y.2.g.5 — Pending Aging** — `pL1PendingAccount` (data-value) + `pL1PendingType` / `pL1PendingRail` (enums).
    - [x] **Y.2.g.6 — Unbundled Aging** — same three over `<prefix>_stuck_unbundled` (`pL1Unbundled{Account,Type,Rail}`).
    - [x] **Y.2.g.7 — Today's Exceptions** — `pL1TodaysExcCheckType` (enum) + `pL1TodaysExcAccount` (data-value) + `pL1TodaysExcType` (enum; SQL adds `OR transfer_type IS NULL` — NULL for 5 of 7 matview branches).
    - [x] **Y.2.g.8 — Transactions** — 5 dropdowns: `pL1TxAccount` / `pL1TxTransferId` / `pL1TxStatus` / `pL1TxOrigin` (data-value; status/origin open-set → options from `DS_L1_TX_FACETS`, transfer from `DS_L1_TX_IDS`) + `pL1TxType` (enum). NOTE: new transfer-filter param is `pL1TxTransferId`, distinct from the M.2b.7 drill sentinel `pL1TxTransfer` (which keeps narrowing the table via its calc-field FilterGroup); the two compose.
    - [x] **Y.2.g.9 — Daily Statement** — `P_L1_DS_ACCOUNT` now bridges to a `pL1DsAccount` SINGLE_VALUED dataset param on both `_daily_statement_summary` + `_daily_statement_transactions` (`WHERE account_id = <<$pL1DsAccount>>`, sentinel default → empty until picked); dropped the two `CategoryFilter.with_parameter` account FilterGroups; repointed the account dropdown's `LinkedValues` source at `DS_L1_ACCOUNTS` (unparameterized) so it still browses all accounts. Date pickers stay analysis-level (Y.2.f territory).
    - [x] **Y.2.g.10 — Aurora deploy + browser-verify** — `database-2` booted, sasquatch_pr re-seeded + matview-refreshed, all 4 apps deployed (commit `a1fa27c` + lifespan-fix `b19137b`); operator manual QuickSight pass: dropdowns narrow as designed AND **noticeably faster** (the headline win — narrowing pushed into indexed `WHERE col IN (<<$p>>)` predicates against the matview vs the old lazy `tenK-sample-values-V2` cold-fetch round-trip). App2-local L1 also serving (drift KPI live-fetch confirmed `{"value":10}` from `sasquatch_pr_drift`); rendering bugs surfaced are pre-existing X.2.g territory, captured in tech debt.
    - [x] **Y.2.g.11 — Quirks-log entry: nothing new** — the X.1.g cold-fetch QS footgun this work fixed is already documented in `docs/reference/quicksight-quirks.md`; no fresh QS misbehavior surfaced during the deploy/verify pass. Final commit + tick.
    Per-sheet cycle: SQL change → dataset-param decls in `build_dataset(...)` → app.py FG-drop + param/dropdown wire (via the g.0 helper) → test fixups (FilterGroup-count + param-count assertions in `tests/json/test_l1_dashboard.py` + siblings) → unit/json suite green → pyright clean → `tests/e2e/test_dataset_sql_smoke.py` green → tick the sub-bullet.
- [x] **Y.2.h — Executives Active Accounts dataset split — DONE (2026-05-10).** Walked the Executives app for filters that benefit from pushdown. Only Account Coverage's `Active Accounts` KPI + `Active Accounts by Type` bar failed both renderers' parity check — they relied on a visual-pinned `NumericRangeFilter` (`activity_count >= 1`) which QS applied but App2 didn't. Fix per K.4.8k / Y.2.a roots-companion split-dataset pattern: split the SQL into two datasets sharing one contract — `exec-account-summary-ds` stays the date-independent snapshot (drives Total Open Accounts + Open Accounts by Type + the unaggregated detail table) and `exec-account-summary-active-ds` (Y.2.h) bakes `WHERE COALESCE(activity_count, 0) > 0` into the SQL plus the X.2.g.1.b `app2_date_filter` dual-SQL pattern so the date-range picker narrows both renderers. Re-pointed Active KPI + bar at the new dataset; dropped `_wire_account_coverage_filter_groups` + `_FG_EXEC_ACCT_ACTIVE_ONLY` + the now-unused `_AutoSentinel` import; added a `TimeRangeFilter` FG scoping `ds_acct_active.last_activity_date` so QS's analysis-level date-range picker narrows the active KPI as well. The Total Open Accounts KPI is now (correctly) excluded from the heuristic — its registered SQL has no `:date_from` because the snapshot is date-independent by design (`build_account_summary_dataset` no longer passes `app2_sql=`). 1807 unit + json tests green.
    - [x] **Y.2.h.1 — Re-enable `test_date_filter_narrows_every_date_sensitive_count_kpi`** — `@pytest.mark.xfail` decorator dropped at `tests/e2e/test_html2_executives_live.py:292`. Heuristic now picks up the active variant (its `app2_sql` carries the `:date_from` bind) and excludes the snapshot variant (its `sql` has no bind), so the test should pass once the chain runs against a live deploy.

### Y.2.gate — Test layer chain audit + runner — DONE (a–o; v8.8.0a6 / a17 / a19 / a20)

The hard prereq for Y.2.c+. Built `./run_tests.sh` (`src/quicksight_gen/_dev/runner.py`) — the layered chain runner (`unit → db → app2 → deploy → api → browser`, `up_to=<layer>` runs through that layer stop-on-first-failure; variant matrix `scenario × dialect × target` = `sp`/`sq`/`fuzz[:N]`/`us:<path>` × `pg`/`or`/`sl` × `lo`/`aw`, 13-cell default, sub-flags compose, `--variants=<code>` triage; `unit` runs once per invocation as a prelude not per cell; cfg-driven auth via `run/config.<dialect>.yaml::auth:`; iteration knobs `--only`/`--parallel`/`--skip-cheap`/`--coverage`; ephemeral AWS via `up`/`down`/`status`; pre-dispatch probes; per-run `timings.json`/`hashes.json` drift detection) that CI's `ci.yml`/`e2e.yml` test jobs now wrap. **Operator contract: CLAUDE.md `Test sequencing + git hooks`** ("never invoke `pytest tests/e2e/` directly for layered work" — the Y.2.b deployed-SQL bug is the cost example). Full a–o detail + the gory bits in `PLAN_ARCHIVE.md` (`# PLAN — Y.2.gate`), `docs/audits/y_2_gate_*.md`, `RELEASE_NOTES.md` v8.8.0a6/a17/a19/a20, git @ `a7cf58f`.
...
- [x] **Y.3.a — Recipient Fanout: `distinct_senders` window column — DONE (2026-05-10).** Pushed `COUNT(DISTINCT o.sender_account_id) OVER (PARTITION BY i.recipient_account_id) AS distinct_senders` into the `joined` CTE; outer `SELECT * FROM joined WHERE distinct_senders >= <<$pInvFanoutThreshold>>` does the threshold pushdown. Added `IntegerDatasetParameter` declaration + `MappedDataSetParameters` bridge from the analysis-level `pInvFanoutThreshold` slider param. Added `distinct_senders INTEGER` to `RECIPIENT_FANOUT_CONTRACT`. Re-pointed the table's `Measure.max(ds_fanout, distinct_senders_calc)` → `ds_fanout["distinct_senders"].max()`. Dropped `CF_INV_FANOUT_DISTINCT_SENDERS` calc field declaration + `FG_INV_FANOUT_THRESHOLD` analysis-level NumericRangeFilter — both QS and App2 now see one shape. Test sweep: dropped `test_threshold_filter_is_parameter_bound_on_calc_field`, replaced with `test_fanout_threshold_pushed_into_dataset_sql`; pivoted `test_distinct_sender_calc_field_declared_at_analysis_level` to `test_distinct_sender_calc_field_dropped_in_y3a` (guard test that the calc field doesn't come back); fixed FG/CF counts in `test_fanout_sheet_serializes_to_aws_json` (FG 6→5, CF 4→3). 1807 unit + json tests green.
- [x] **Y.3.b — Account Network: `is_inbound_edge` + `is_outbound_edge` + `counterparty_display` window columns — DONE (2026-05-10).** New `ACCOUNT_NETWORK_CONTRACT` extends `MONEY_TRAIL_CONTRACT` with the three columns. Dataset SQL adds three CASE expressions over `<<$pInvANetworkAnchor>>` to the outer SELECT (`base.*` projects the inherited columns; the three CASE expressions add the anchor-derived ones). Money Trail dataset keeps `MONEY_TRAIL_CONTRACT` since it has no anchor concept. Rolled up Y.3.d (visual repointing — calc field refs collapsed to `ds_anet["..."]` reads in 4 lines) and Y.3.e (CalcField sweep — dropped 3 `add_calc_field` calls, dropped `CalcField` / `Dim` / 3 `CF_INV_ANETWORK_*` imports). CalcFields list is now empty for the Investigation analysis; FilterGroups for inbound/outbound Sankey direction now target real columns; the table's walk-the-flow drill source uses `counterparty_display.dim()` (was `Dim(ds_anet, counterparty_display_calc)`). 1805 unit + json tests green.
- [x] **Y.3.c — Volume Anomalies calc fields — NO-OP (2026-05-10).** Audit confirmed Volume Anomalies has no analysis-level CalcFields. The `z_score_max = ds_anomalies["z_score"].max()` line is a `Measure.max()` aggregation on an existing real column, not a CalcField declaration. Nothing to push down.
- [x] **Y.3.d — Update visuals to reference real columns — DONE (2026-05-10).** Folded into Y.3.a (Recipient Fanout: `Measure.max(ds, distinct_senders_calc)` → `ds_fanout["distinct_senders"].max()`) and Y.3.b (Account Network: 3 calc field refs → 3 `ds_anet["..."]` reads).
- [x] **Y.3.e — Drop CalcField declarations — DONE (2026-05-10).** Folded into Y.3.a (dropped `CF_INV_FANOUT_DISTINCT_SENDERS` add_calc_field) and Y.3.b (dropped 3 anetwork add_calc_field calls). Investigation analysis tree now emits zero CalcFields.
...
- [x] **Y.5.a — `app2_sql=` → `app2_date_column=` refactor (2026-05-10).** Replaced the raw-SQL escape hatch with a typed declaration. Pre-refactor every callsite (10 total: 8 L1 + 2 Executives) hand-rolled `app2_sql=template.format(date_filter=app2_date_filter("col", cfg.dialect))` — boilerplate that broke silently when an operator forgot to wire the App2 variant. Post-refactor each declares `app2_date_column="col"` and `build_dataset()` does the substitution + registration internally. The build_dataset signature is now positionally compatible (sql still a positional arg; `app2_date_column` is keyword-only). The `{date_filter}` template slot stays because it's a clear seam, but the .format() machinery is centralized. The Y.2.h Executives active variant pre-substitutes its `{active_only}` slot via .replace before passing the template to build_dataset. `app2_date_filter` import dropped from both `apps/l1_dashboard/datasets.py` and `apps/executives/datasets.py` (only `build_dataset()` imports it now, lazily). Verified: 1979 unit + json + data passed, pyright clean.
