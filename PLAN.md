...
- [ ] **X.4.j.1** — Cross-dialect pull tests: PG → SQLite (primary), Oracle → SQLite, SQLite → SQLite (degenerate). NOT a full matrix — the codebase keeps the dialect support; we just don't fan it out for Studio's tests.
  ...
  - [ ] **X.4.j.1.a** — Postgres container fixture: reuse the runner's `seed_variant` / `local-pg` plumbing (`runner.py::setup_variant("pg", "lo")`) instead of a parallel docker-compose. Returns `(container_url, cleanup_fn)`. Container lifecycle is per-test (function-scoped) so concurrent test runs don't collide.
  - [ ] **X.4.j.1.b** — etl_hook fixture: writes a tiny shell script (`#!/bin/bash; .venv/bin/quicksight-gen data apply --execute -c <generated-pg-cfg> --l2 <yaml>`) to a tempfile + chmod +x. Sasquatch_pr is the L2; the cfg is generated per-test pointing at the postgres container. Hook seeds postgres on every pipeline run — proves the gate fires + step 2 sees fresh source data.
  - [ ] **X.4.j.1.c** — Studio process spawn: in-process via `make_app(... studio_routes=make_studio_routes(cache, cfg=...))` + httpx AsyncClient (NOT a subprocess). Faster + matches the existing `tests/unit/test_studio_*.py` pattern. Live drive (X.4.j.4) is the subprocess path.
  - [ ] **X.4.j.1.d** — POST /deploy assertion: hit the endpoint, parse the DeploySummary JSON, assert `halted=False`, `step1_etl_hook_exit_code=0`, `step2_pull_transactions_pulled > 0`, `step3_generator_transactions_after >= step2_pull_transactions_pulled`, `step4_matviews_done=True`, `step5_data_generation_id` bumped.
  - [ ] **X.4.j.1.e** — Post-deploy SQLite verification: open the sqlite tempfile directly + assert `<prefix>_transactions` is non-empty AND `<prefix>_drift` matview has rows (proves step 4 ran AGAINST the post-step-3 base data).
  - [ ] **X.4.j.1.f** — Dashboards round-trip: hit `GET /dashboards/<id>/sheets/<sheet>/visuals/<visual>/data` for one canary visual on EACH of the 4 dashboards (L1 / L2FT / Investigation / Executives) — assert the JSON payload is non-empty for every dashboard. Catches "deploy succeeded but app X's datasets misread the post-step-3 state" regressions on the spot, not later in the dev loop.
  - [ ] **X.4.j.1.g** — Halt-on-failed-hook integration test: same fixture but the hook script `exit 1`s. Assert `halted=True`, sqlite tempfile stays empty (no wipe occurred).
  - [ ] **X.4.j.1.h** — Layer + gating: lives in `tests/e2e/test_deploy_pipeline_pg_to_sqlite.py`, gated by `QS_GEN_E2E=1` AND probe `docker ps`. Runner exposes via the `app2` layer (closest fit — spins local servers, no AWS). NOT in `unit/` (postgres container takes ~5s to spin up).
- [ ] **X.4.j.2** — Browser e2e: Studio + Dashboards integrated loop on `sasquatch_pr` + SQLite (edit YAML in Studio → Deploy → Dashboards reloads with the new data). Single test, NOT parametrized over `[qs, app2]`.
  - Shape: builds on X.4.j.1.a-c fixtures + adds Playwright. Drives the actual button click + asserts the status indicator transitions running → ok with the right tx count visible. Then navigates to /dashboards and asserts a visual renders non-empty.
  - [ ] **X.4.j.2.a** — Playwright fixture wired against the in-process httpx app (Starlette + uvicorn subprocess for the real bind, since Playwright can't talk to TestClient). Reuse the existing `App2Driver.serving` pattern.
  - [ ] **X.4.j.2.b** — Click Deploy, wait for `#deploy-status.deploy-status--ok` (or `.deploy-status--halted` on the negative test), assert the status text contains `Deployed (gen N, M tx)`.
  - [ ] **X.4.j.2.c** — Cross-page navigation: open EACH of the 4 dashboards (L1 / L2FT / Investigation / Executives), navigate to one canary sheet per app, assert at least one visual renders non-empty. Reuses the X.4.j.1.f canary visuals — single source of truth for "what proves this app's data wired through".
  - [ ] **X.4.j.2.d** — Auto-reload via the data_generation_id poller (X.4.g.12.b): open dashboard A in tab 1, fire POST /deploy from a separate context, assert tab 1 reloads itself within ~6s without manual refresh.
