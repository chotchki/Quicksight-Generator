...
- [ ] **Y.2.gate.b — Design the runner.** Single script (`./run_tests.sh up_to=<layer>`?  `tox`-style?  Make targets?) that composes the chain. Decisions:
    ...
    - **Variants axis (per layer)**: Each layer can be parameterized — same layer × N variants. The runner multiplexes variants and rolls failure status per (layer, variant) cell. Variants we know about today:
        ...
        - **L2 instance**: static — `spec_example` (default) / `sasquatch_pr` (CI) / others as added. Most layers care; layer 1 doesn't.
        - **Fuzz seed (property-testing axis — first-class, LOCKED 2026-05-07)**: `tests/l2/fuzz.py::random_l2_yaml(seed)` produces a different valid L2 topology per seed. **Default = 1 random seed per run** (different on each invocation); operator opts in to larger samples via `--fuzz-seeds=N`, typical for nightly cron + pre-release runs. Cumulative coverage emerges naturally over many runs without per-run cost spikes. A previously-green seed that fails on a new run is signal — same drift-detection shape as timings + hash-lock.
        - **Browser e2e target**: `qs | app2`. Layer 6 (browser e2e) merges with the old layer 7 — same conceptual layer, different render targets. Both targets run in parallel with fast-fail (LOCKED §7.10 / §7.12 refinement: "the main end deliverable is still AWS QuickSight; App2 + QS run in parallel, not sequential").
        - **Future**: per-app subset (`--apps=l1,inv`), per-region.
    - **Container-backed isolation for local-DB variants**: Layer 3a/3b dialect variants run in per-variant Docker containers (PG, Oracle), not against the live Aurora. Local dev box is unconstrained; containers give us fixture-data byte-identity per run + no shared-state contamination. `testcontainers-python` is the leading candidate (typed, cleanup on exit). The Aurora live cell is reserved for layer 4+ (deploy / e2e).
    - **App2 promoted to the early e2e gate** (user comment, addressed in audit §7.10): Don't expose local Docker to QS via VPC tunnel — heavy infra, doesn't actually relieve the QS rate-limit. Instead, slot App2 (layer 7) **before** the QS deploy as **layer 3.7**: same dataset SQL, runs against local Docker, no AWS contact, catches ~80% of the bug classes (dialect SQL, pushdown substitution, calc-field-to-SQL translation). QS layer 4-6 demotes to nightly/pre-release parity check for the remaining ~20% (QS-side rendering, embed signing, dashboard structure). Strategic reprioritization → likely owned by X.2.j; Y.2.gate.b just slots it in the chain.
    - **Per-run output isolation**: Every artifact lands under `runs/<run-id>/...` (run-id = `<utc-ts>-<short-sha>[-dirty]`). No more shared `run/out/`, `tests/e2e/screenshots/`, `tests/e2e/failures/`, `.coverage.*`, container logs. Browser test JS console + Playwright network traces / HAR / video / screenshots also land here (failure-only by default; `--trace-all` for noisy debugging). **Every L2 YAML always copied — fuzzed AND static** (LOCKED 2026-05-07): every L2 instance the run touches lands at `runs/<run-id>/l2/<instance-or-seed>.yaml`. Catches drift on static fixtures (`spec_example` / `sasquatch_pr` edits between runs) the same way it preserves fuzzed-seed YAML for failure reproduction. Last N=20 runs kept; auto-prune. `runs/` is gitignored. **"Disk is cheap compared to time"** — capture liberally; the friction of "I wish I had X from the failed run" dwarfs the storage cost.
    - **Timing capture + diff vs prior run**: Runner records per-(layer, variant, test) timings to `runs/<run-id>/timings.json`. On next invocation, reads the most-recent matching prior run and reports `step took X (was Y, ±Z%)`. Highlight (⚠) threshold: ±50% change (configurable). Same shape as hash-locked seed data. **The hash lock collapses into the same `runs/<run-id>/` capture/diff loop** (LOCKED 2026-05-07): hash + timings get diffed together against the prior run; "single-source-of-drift" principle — any stable artifact (timing, seed hash, output bytes, row counts) lives in `runs/<run-id>/` and gets diffed.
    - **CI uploads `timings.json` as a workflow artifact** (LOCKED 2026-05-07) so cross-CI-run drift is visible. Same shape as existing `coverage-data-py3.13` / `e2e-pg-api-top-queries-${run_id}` artifacts.
...
- [ ] **Y.2.gate.c — Implement the runner.** Shell script (LOCKED §7.3 — not Click subcommand; this is dev-tooling, not customer-facing surface). Concrete deliverables (per Y.2.gate.b spec):
    - Per-run `runs/<run-id>/` directory created at session start; all artifacts written there (incl. browser traces, hash lock, timings).
    - `runs/<run-id>/timings.json` + `runs/<run-id>/hashes.json` (or merged) recorded per (layer, variant, test). Diffed against prior run on next invocation.
    - On invocation, scan prior `runs/*/timings.json` for matching SHA (else most-recent), report `step took X (was Y, ±Z%)` with ⚠ threshold.
    - `runs/` added to `.gitignore`; auto-prune past N=20.
    - Stop-on-first-failure across layers; xdist within layer; **App2 + QS targets run in parallel at layer 6** with fast-fail.
    - `--up-to=<layer>`, `--variants=<set>`, `--only=<test-id>`, `--skip-cheap`, `--fuzz-seeds=N` flags per Y.2.gate.b.
    - **No state file** (LOCKED §7.12). Layer dispatch probes the actual dependency (DB connection, container daemon) — stale state files are a footgun the user explicitly rejected.
...
- [ ] **Y.2.gate.h — Credential auto-discovery.** Operators / Claude should never have to look up or pass credentials by hand. Any credential the runner needs gets resolved automatically with a clear error pointing at the missing source if discovery fails. Concrete cases today:
    ...
    - **AWS account / region / partition** — already in cfg; same.
    ...
    - **Loud failure on missing config** (LOCKED §7.2 2026-05-07): if the runner config is missing required values (cfg yaml, AWS creds, etc.), the runner fails loudly with a clear pointer at the missing item — not a polite skip.
...
- [ ] **Y.2.gate.j — Parallelism (so the longer chain doesn't kill iteration speed).** Total wall-clock matters or developers will skip the chain to iterate. Strategy:
    - **Local layers (1-3) are unconstrained.** User runs on a beefy Mac and Docker is fair game. The bottleneck is QuickSight (AWS API throttle, deploy churn, embed signing) — and that bottleneck is **self-relieving as Phase Y lands** (fewer + smaller queries per visual). Re-evaluate parallelism budgets after each sweep.
    - **Cross-layer = sequential** (chain semantics require this). No pipelining; complicates failure attribution.
    - **Variants of one layer = parallel via per-variant containers.** Layer 3a runs PG container + Oracle container + SQLite (in-process) simultaneously — `testcontainers-python` or `docker compose up` per variant; no shared connection contention. Layer 4-6 (AWS-touching) parallelize across `L2Instance` tag = separate AWS resource sets, throttle-aware.
    - **Fuzz-seed parallelism (when sample > 1)** (audit §7.11). Default per LOCKED 2026-05-07 is 1 random seed/run; cumulative coverage emerges over runs. When the operator opts into a larger sample (`--fuzz-seeds=100`, typical for nightly), the per-seed local Docker × xdist parallelism completes in roughly the wall-clock of one sequential seed run. A regression is reproducible by pinning `QS_GEN_FUZZ_SEED=<N>` from the failing-seed manifest.
    - **Container reuse policy**: PG containers cheap (~5s start) — recreate per session OK. Oracle 19c heavy (~30-60s, ~5GB) — name-cache across runs in the same session (`quicksight-test-oracle`), recreate only on schema drift. SQLite is in-process; no container.
    ...
    - **Wall-clock budget targets**: layer 1+2+3a should land < 60s. Through layer 4 (deploy) < 5min. Through layer 6 (full e2e) < 15min single-dialect, < 25min full matrix. **Targets get tighter as Y.2 progresses** — re-baseline after each major sweep. If we drift past these we revisit (most likely culprit: QS API throttle, not local CPU).
    - **Hash-locked seed + ephemeral DB = no flaky-state debt.** Every test sees byte-identical fixture data because the seed is hash-locked AND the container is fresh. The M.4.4.12-style xfail-as-flake category should shrink.
- [ ] **Y.2.gate.k — CI parity (no `works-on-my-box` surprises).** The same runner runs locally and in CI. Decisions:
    ...
    - **e2e-oracle-browser cell added to nightly cron** (LOCKED §7.4 2026-05-07): PG and Oracle should be at parity for browser e2e — the asymmetry today is a gap, not a deliberate choice.
    ...
    - **`timings.json` uploaded as a workflow artifact** (LOCKED §7.9 2026-05-07) so cross-CI-run drift is comparable.
...
- [ ] **Y.2.gate.l — Ephemeral AWS infra (start/stop both ways).** Per audit §7.12. **Motivator: pain visibility, not dollar cost** (user: "I don't care about the running cost, I have a sense of the pain"). Split:
    - **CI side (LOCKED 2026-05-07 — start/stop the existing cluster, not create-per-job)**: each AWS-touching CI job (`e2e.yml::e2e-pg-{api,browser}`, `e2e.yml::e2e-oracle-{api,browser}`, `release.yml::e2e-against-testpypi`) gains pre-test `aws rds start-db-cluster` + `if: always()` post-test `aws rds stop-db-cluster`. Storage cost continues; compute drops to zero between runs; preserves seed state across CI invocations.
    ...
    - **Layer dispatch interlock probes the actual dependency** (LOCKED 2026-05-07): when the runner is asked for a layer requiring AWS or Docker, it probes the live connection (e.g. `aws sts get-caller-identity`, `psycopg.connect`, `docker ps`). Stale state files are a known footgun the user explicitly rejected; runtime check is authoritative. Failure: `"layer 4 needs AWS DB but cluster is stopped — run './run_tests.sh up' first or invoke './run_tests.sh up_to=app2-e2e' to skip"`.
    - **No state file** (LOCKED 2026-05-07). Probe is cheap; no hidden state to drift.
    - **No auto-stop timer** (LOCKED 2026-05-07 — explicitly rejected). Operator's `down` discipline + `status --cost` visibility keep things tidy.
    - **App2 + QS run in parallel at layer 6** (LOCKED 2026-05-07 — refines §7.10): "the main end deliverable is still AWS QuickSight." App2 + QS browser tests run concurrently with fast-fail; not "App2 first, QS later". App2's speed advantage is in the local-Docker layers (3 + the App2-target portions of 6); at the AWS-touching steps both targets run side-by-side.
- [ ] **Y.2.gate.g — `Y.2.c+ unblocked` gate.** Once a-l are done, resume the Y.2 sweep. Note in PLAN that the gate fires here.
