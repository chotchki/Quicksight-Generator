...
- [ ] **Y.2.g — L1 per-sheet *categorical* filter controls.** *(Audit done 2026-05-10 — see breakdown below; data-value-column approach pending a user call.)* Convert `apps/l1_dashboard/app.py::_wire_per_sheet_category_filters`'s ~22 per-sheet dropdowns from `CategoryFilter.with_values(values=[], "FILTER_ALL_VALUES")` (the X.1.g cold-fetch footgun — these lazy-fetch the column's distinct values from QS's `tenK-sample-values-V2` endpoint, which 404s on cold per-CI-run dashboards) to dataset-SQL pushdown. Converting fixes the footgun *and* the pushdown together. Plus Daily Statement's 2 account FilterGroups (already analysis-`StringParam`-bound via `CategoryFilter.with_parameter`, but not yet pushed down) and its date pickers (those are Y.2.f / date territory — leave). **The ~22 split into two sub-patterns:**
    ...
    - **Data-value columns (~9)** — `account_id` (Drift / Overdraft / Limit Breach / Pending / Unbundled / Today's Exceptions / Transactions / Daily Statement ×2) and `transfer_id` (Transactions). No static enum → can't be `StaticValues`. Needs the Y.2.a/b companion-dataset pattern (a small unfiltered `SELECT DISTINCT account_id, account_name FROM <prefix>_transactions` companion dataset; dropdown's `LinkedValues.from_column(companion, ...)`; the dropdown's `StringParam` (single? multi?) bridges to a `<<$pAccount>>` in each filtered dataset's SQL). **OPEN — pick one before writing the per-sheet sub-bullets:** (a) one shared `<prefix>_l1_accounts` companion dataset that every `account_id` dropdown's `LinkedValues` points at (+ a `<prefix>_l1_tx_ids` companion for `transfer_id`) — cleanest, ~2 new datasets; (b) per-sheet companion datasets (one `SELECT DISTINCT account_id FROM <that-sheet's-matview>` each) — narrower option lists but ~8 new datasets; (c) do the enum-column conversions now (unambiguous), defer the data-value ones to a follow-up — gets ~13 of the ~22 done + fixes the footgun on those, leaves `account_id`/`transfer_id` on the lazy-fetch path for now. **Recommend (a)** for the data-value ones (one companion dataset, broadest-but-still-correct option list) — but (c) is a fine "do the easy 13 first" wedge.
    - **Sub-bullets (g.0 + per-sheet) to be written once the data-value approach is locked:** g.0 — build an L1 `_populate_pushdown_dropdown` helper (mirror `apps/l2_flow_tracing/app.py`'s) + the enum-value helpers (`l1_rail_values(l2)` / `l1_transfer_type_values(l2)` / `l1_account_role_values(l2)` / `l1_supersede_reason_values()` / `l1_check_type_values()`); g.1 Supersession Audit (1 enum filter — simplest first cut); g.2 Limit Breach (1 enum + 1 data-value — exercises both); g.3 Drift + Drift Timelines (ALL_DATASETS case); g.4 Overdraft; g.5 Pending Aging (3 filters); g.6 Unbundled Aging (3); g.7 Today's Exceptions (3); g.8 Transactions (5 — incl. `transfer_id` data-value); g.9 Daily Statement (the 2 account FGs → push their existing param down; date pickers stay); g.10 Aurora deploy + browser-verify each sheet's dropdowns narrow + empty-multi-select reverts to all; g.11 quirks-log + commit. Each sheet: SQL change → dataset-param decls → app.py FG-drop + param/dropdown wire → test fixups (FG counts, param counts) → unit/json green → pyright clean → `tests/e2e/test_dataset_sql_smoke.py` green → tick.
