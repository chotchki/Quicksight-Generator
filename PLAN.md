...
- [ ] **Y.2.gate.b — Design the runner.** Single script (`./run_tests.sh up_to=<layer>`?  `tox`-style?  Make targets?) that composes the chain. Decisions:
    ...
    - **Per-run output isolation**: Every artifact lands under `runs/<run-id>/...` (run-id = `<utc-ts>-<short-sha>[-dirty]`). No more shared `run/out/`, `tests/e2e/screenshots/`, `tests/e2e/failures/`, `.coverage.*`, container logs. Browser test JS console + Playwright network traces / HAR / video / screenshots also land here (failure-only by default; `--trace-all` for noisy debugging). Last N=20 runs kept; auto-prune. `runs/` is gitignored.
