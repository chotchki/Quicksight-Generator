...
- [ ] **M.1 — L2 library foundation.** Lift the spike's working glue into proper library modules. Typed YAML loader matching SPEC; all SPEC validation rules with rejection tests; prefix-aware SQL emission; Current* projection. Acceptance: `from quicksight_gen.l2 import load_instance, validate, ...` works; spike's `slice.yaml` loads through it cleanly; pyright strict on `common/l2/`.
  ...
  - [ ] M.1.2 — YAML loader with friendly error messages (cite file/line for validation failures). **Per F4:** centralize Money coercion in a `_load_money(raw)` helper that uses `Decimal(str(value))` to dodge YAML float precision; apply uniformly across every Money-typed field. **Per F5 (SPEC-pinned):** validate `InstancePrefix` against `^[a-z][a-z0-9_]*$` + 30-char cap as a centralized identifier-format helper; reuse for any other identifier-typed field.
