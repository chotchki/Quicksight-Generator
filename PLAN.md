...
- [ ] **Y.2.gate.c — Implement the runner.** Substrate: pytest + `quicksight_gen/_dev/runner.py` (~400 LOC budget) + `./run_tests.sh` bash shim (locked by `Y.2.gate.b.0` spike). Each sub-task carries the module / function / hook ownership.
    ...
    - [ ] **Y.2.gate.c.6 — Within-layer parallelism** (split 2026-05-07 into c.6.parallel done + c.6.async still open).
        ...
        - [ ] **Y.2.gate.c.6.async — Cross-variant `asyncio.gather` fan-out.** When real variants exist (b.2 testcontainers per-dialect; b.3 App2-as-early-gate; merged 6/7 with `target=app2|qs`), the orchestrator runs per-variant pytest subprocesses via `asyncio.gather(*tasks, return_exceptions=False)` for fast-fail. **First real consumer is b.2** (no c-stage task strictly forces it today — deploy/api/browser are stubs; cross-variant has no second variant to gather across). Don't build speculative async infra here; lands when b.2 introduces the first real second variant.
