...
- [ ] **U.7.5 — Extract provenance + pdf-chrome modules.** `cli/audit.py` is at ~4400 lines. Two self-contained chunks ready to lift: **(a) `common/provenance.py`** — `ProvenanceFingerprint` dataclass + `_canonical_value` + `_hash_table_rows` + `_l2_yaml_sha256` + `_quicksight_gen_code_identity` + `_compute_provenance` + the two placeholder helpers (~250 lines). Generic — knows about base tables + L2 yaml + code identity, zero audit-specific rendering. Reusable by `audit verify` (lighter import) and any future tool that wants to bind output to source data. **(b) `common/pdf/audit_chrome.py`** — `BookmarkedDocTemplate` (renamed from `_AuditDocTemplate`) + `bookmarked_h1`/`bookmarked_h3` + `make_footer_drawer` factory (~150 lines). Generic reportlab plumbing — `afterFlowable` for outline+TOC, `_allSatisfied` override for `total_pages_holder`, page-template chrome closure. Stays in audit.py: `_provenance_block_story` and `_signoff_story` (audit-specific content), the per-section `_*_story` builders, the dataclasses, the `_query_*` functions, the markdown renderers. After this, audit.py is ~3850 lines — still big but cleanly split between "what the audit report contains" (audit-specific) and "how the PDF is built" (reusable). Defer the bigger `cli/audit/` package split (queries.py / markdown.py / pdf.py / __init__.py) to a later pass.
- [ ] **U.7.b — Auto-sign PDF (key sourced from config.yaml).** Per the U.5 sign-off block design, the system attestation block is the cryptographically-signable artifact. U.7.b: when `config.yaml` carries a `signing:` block (key path, key passphrase env var, optional cert chain), `audit apply --execute` automatically applies a PDF-level signature over the PDF bytes (or specifically over the `/Subject` metadata + system attestation page bytes, TBD). The auditor block stays unsigned — a human can sign it later in print without invalidating the system seal. When `signing:` is absent, the PDF ships unsigned (current behavior). Library options: `pyHanko` (best-of-class for reportlab-generated PDFs, supports PAdES + LTV); `endesive`. Investigation step: pick library, scope key formats to support (PEM RSA + PKCS#11 token?). Note Phase V's config/L2 split tightens what should live in `config.yaml` — signing keys are operator infrastructure, NOT L2-specification, so they belong in config.yaml.
  - Would like to automatically sign the whole PDF if possible while still allowing for a second and/or third digital signature (the document should be silent on how many are required). All the lines for name / title /date aren't needed (the pdf signature covers that). Letting the notes box be a fillable form before the second signature would be nice touch so comments can be added before signing and locking it.
  - Plan on a PEM RSA block
- [ ] **U.7.c — Provenance appendix page (matview hashes + verify recipe).** New back-of-PDF appendix page (after Sign-off, bookmarked at level 0) covering everything an auditor or third-party verifier needs to independently re-derive the report's bindings: (a) **Matview SHA256 sidecar** — the per-matview row hashes (drift, overdraft, limit_breach, stuck_pending, stuck_unbundled, daily_statement_summary, supersession-relevant matviews) computed at audit time. Distinct from the authoritative composite fingerprint (which covers base tables, not matviews). When these diverge from a recompute, that's a *technical* signal (matview needs refresh, schema drift) rather than a data-binding problem. (b) **Setup commands** — exact shell + SQL the verifier runs to reproduce: `quicksight-gen audit verify report.pdf -c config.yaml --l2 path` for the one-shot check, plus the manual SQL the verify command runs (the alphabetical-discovery `SELECT * FROM <prefix>_transactions WHERE entry <= <hwm>` and the row-canonicalization rules). (c) **Per-source recompute formulas** — how to manually SHA256 each input independently (transactions, daily_balances, L2 yaml, code identity). The page should be enough that someone WITHOUT quicksight-gen installed could write 50 lines of Python to verify the fingerprint. Bookmark + TOC entry at level 0. Targets the regulator who wants to audit the auditor.
- [ ] **U.8 — Test layer.**
