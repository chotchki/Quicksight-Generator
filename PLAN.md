...
- [ ] **M.1 — L2 library foundation.** Lift the spike's working glue into proper library modules. Typed YAML loader matching SPEC; all SPEC validation rules with rejection tests; prefix-aware SQL emission; Current* projection. Acceptance: `from quicksight_gen.l2 import load_instance, validate, ...` works; spike's `slice.yaml` loads through it cleanly; pyright strict on `common/l2/`.
  - [ ] M.1.1 — Typed dataclasses for L2 primitives (Account, AccountTemplate, Rail, TransferTemplate, ChainEntry, LimitSchedule, …) matching the SPEC tuples 1:1. **Per F2:** model `Rail` as a discriminated union (`TwoLegRail` / `SingleLegRail`) rather than one dataclass with all fields nullable — pyright catches "leg_role on a two-leg rail" at the wiring site instead of at load. **Per F6:** rename one of the two `Dataset` types to remove the api-`DataSet` vs tree-`Dataset` collision (recommend `DataSetModel` for the api shape; tree's `Dataset` stays).
