...
- [ ] **Y.2.g — L1 per-sheet *categorical* filter controls.** *(Audit done 2026-05-10 — see breakdown below; data-value-column approach pending a user call.)* Convert `apps/l1_dashboard/app.py::_wire_per_sheet_category_filters`'s ~22 per-sheet dropdowns from `CategoryFilter.with_values(values=[], "FILTER_ALL_VALUES")` (the X.1.g cold-fetch footgun — these lazy-fetch the column's distinct values from QS's `tenK-sample-values-V2` endpoint, which 404s on cold per-CI-run dashboards) to dataset-SQL pushdown. Converting fixes the footgun *and* the pushdown together. Plus Daily Statement's 2 account FilterGroups (already analysis-`StringParam`-bound via `CategoryFilter.with_parameter`, but not yet pushed down) and its date pickers (those are Y.2.f / date territory — leave). **The ~22 split into two sub-patterns:**
    ...
    - [ ] **Y.2.g.0 — Foundation.** (a) L1 `_populate_pushdown_dropdown` helper in `apps/l1_dashboard/app.py` (mirror `apps/l2_flow_tracing/app.py`'s — analysis `StringParam` multi_valued default=all-values + `mapped_dataset_params=bridges` + `add_parameter_dropdown(StaticValues, MULTI_SELECT)`, or a `linked_values` variant for the data-value dropdowns). (b) enum-value helpers: `l1_rail_values(l2)` / `l1_transfer_type_values(l2)` / `l1_account_role_values(l2)` (walk the L2 instance), `l1_supersede_reason_values()` (the `SupersedeReason` enum), `l1_check_type_values()` (the 6 L1-invariant names) — in `apps/l1_dashboard/datasets.py` or a sibling. (c) the 2 companion datasets (`build_l1_accounts_dataset`, `build_l1_tx_ids_dataset`) + their `DatasetContract`s + `DS_L1_ACCOUNTS` / `DS_L1_TX_IDS` constants + registration in `build_all_l1_dashboard_datasets`. (d) `PUSHDOWN_NO_MATCH_SENTINEL` reuse for the `IN ('__no_match__')` empty-multi-select fallback. Unit-test the helpers; `tests/e2e/test_dataset_sql_smoke.py` picks up the 2 new companion SQLs.
    - [ ] **Y.2.g.1 — Supersession Audit** (1 enum filter: `supersedes`/`SupersedeReason`). Simplest first cut — establishes the L1 enum pattern.
    - [ ] **Y.2.g.2 — Limit Breach** (1 enum `transfer_type` + 1 data-value `account_id`). Exercises both sub-patterns + the companion-dataset wiring.
    - [ ] **Y.2.g.3 — Drift + Drift Timelines** (the `cross_dataset="ALL_DATASETS"` case — `fg-l1-drift-{account,role}` narrow both the leaf-drift + ledger-drift datasets; `fg-l1-drift-tl-role` narrows both timeline datasets → bridge to BOTH datasets per dropdown, like Y.2.e's TT sheet).
    - [ ] **Y.2.g.4 — Overdraft** (`account_id` data-value + `account_role` enum).
    - [ ] **Y.2.g.5 — Pending Aging** (`account_id` data-value + `transfer_type`/`rail_name` enums).
    - [ ] **Y.2.g.6 — Unbundled Aging** (same 3 over `<prefix>_stuck_unbundled`).
    - [ ] **Y.2.g.7 — Today's Exceptions** (`check_type` enum + `account_id` data-value + `transfer_type` enum).
    - [ ] **Y.2.g.8 — Transactions** (`account_id`/`transfer_id` data-value + `status`/`origin`/`transfer_type` enums — 5 filters; `origin` enum = the per-leg `Origin` enum; `transfer_id` uses the new `<prefix>_l1_tx_ids` companion).
    - [ ] **Y.2.g.9 — Daily Statement** (the 2 already-`StringParam`-bound `account_id` filters → push their existing param down into `<prefix>_l1_daily_statement_{summary,transactions}` SQL + bridge + repoint the dropdown source at `DS_L1_ACCOUNTS`; the date pickers stay — Y.2.f territory, parked).
    - [ ] **Y.2.g.10 — Aurora deploy + browser-verify** each sheet's dropdowns narrow independently + empty-multi-select reverts to "all" (no `IN ()` / no SQL error) — same as Y.2.c.5.
