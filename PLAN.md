...
- [x] **L.1 — Tree primitives in `common/`.** Shipped as the `common/tree/` package (split when it crossed the 600-line trigger in commit `23d5a0b`). Acceptance met: primitives shipped with full unit coverage (699 tests); `to_aws_json()` round-trip test over the kitchen-sink app green; the L.0 spike's Account Network port re-implemented against the full primitive set emits byte-identical JSON (L.1.15).
  ...
  - [ ] L.1.21 — Parent-creates-child API (Bucket A): make 4 runtime checks structurally impossible by tightening the construction surface to match QuickSight's actual constraints. Insight: the QS API surface is the ceiling — encoding QS's restrictions in our wrappers moves bugs from deploy-time to wiring-time at zero cost (if QS expands later, we widen the type). Wordiness pays once at the wrapper definition; bug-catching is recurring. Per the user's framing: "the API getting more prescriptive is a good thing." Sub-steps:
    - L.1.21.1 — `Sheet.add_kpi / add_table / add_bar_chart / add_sankey / add_text_box` — each method takes the visual's construction kwargs PLUS the placement kwargs (col_span, row_span, col_index, row_index=None) and folds construct + register + place into one call. Inline-kwargs signature (not pre-constructed node) so a fresh visual is born owned-and-placed; "duplicate placement" can't arise. Eliminates `Sheet.place` "unregistered" + "duplicate placement" raises (`structure.py:332` + `:338`). Retires `Sheet.add_visual`, `Sheet.place`, `Sheet.place_text_box`, `Sheet.add_visual` PEP-695 generic. `VisualNode` spike-shape factory + `ParameterControlNode` retire (only used by tests; pulling forward from L.2.13).
    - L.1.21.2 — `App.create_dashboard(dashboard_id_suffix=..., name=...)` — uses the App's already-set analysis. Standalone `Dashboard(...)` constructor goes private/internal (or removed entirely). Eliminates `App.set_dashboard` analysis-identity-mismatch raise (`structure.py:674`). Retires `App.set_dashboard`.
    - L.1.21.3 — `Sheet.scope(filter_group, visuals)` — sheet's own identity constrains which visuals are scopable; type checker proves the visuals belong to this sheet via the sheet's `add_*` registration history. Retires `FilterGroup.scope_visuals(sheet, ...)`. `FilterGroup.scope_sheet` (one sheet, no visuals list) stays. Eliminates the "visual not on this sheet" raise (`filters.py:355`).
    - L.1.21.4 — `visual.add_drill(target_sheet=None, writes, name, trigger)` — visual already exists, knows its owning sheet via a private back-ref set by `Sheet.add_*`, so `target_sheet=None` defaults to "this sheet" without the `AUTO` back-fill machinery. Retires `Drill.target_sheet: Sheet | AutoResolved` → becomes `Sheet` (required, set at construction). Removes the resolver branch that back-fills `target_sheet`. Drops the `visual.actions.append(Drill(...))` pattern in caller code.
    - L.1.21.5 — Migration: `apps/investigation/app.py` (~50 call sites), `tests/test_tree.py` (~80 sites), `tests/test_kitchen_app.py` (~30 sites), `tests/e2e/_kitchen_app.py`. Mechanical — no logic change. Hard switch per the no-back-compat-shims rule.
    - L.1.21.6 — Tests: drop the rejection tests for the four eliminated raises (they tested behavior that's now structurally impossible; type checker is the proof). Update L.1.18 audit table to reflect the smaller surface.
    - L.1.21.7 — Run pytest + pyright; CLI JSON should be byte-identical pre/post (the change is API-shape, not emit semantics).
