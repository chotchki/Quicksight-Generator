...
- [ ] **Y.2.gate.b — Design the runner.** Single script (`./run_tests.sh up_to=<layer>`?  `tox`-style?  Make targets?) that composes the chain. Decisions:
    ...
    - **Container-backed isolation for local-DB variants**: Layer 3a/3b dialect variants run in per-variant Docker containers (PG, Oracle), not against the live Aurora. Local dev box is unconstrained; containers give us fixture-data byte-identity per run + no shared-state contamination. `testcontainers-python` is the leading candidate (typed, cleanup on exit). The Aurora live cell is reserved for layer 4+ (deploy / e2e).
      - Additional comment: can we expose the local docker databases to quicksight as an early e2e gate? That would remove the autoscaling contention in AWS and the CI.
