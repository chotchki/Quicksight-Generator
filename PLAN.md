...
- [ ] **P.9f — Oracle e2e cleanups (gates P.10).** First full Oracle e2e cell (oracle × spec_example) revealed 11 failures. Categorized by root cause from `tests/e2e/failures/`:
  ...
  - [ ] P.9f.b — JSON path concat at QS-runtime. QS console shows `ORA-40597: JSON path expression syntax error ('$.')` from the L2FT datasets' `JSON_VALUE(metadata, '$.' || <<$pKey>>)` pattern. Oracle's JSON path parser may need `$."key"` quoted form, OR QS doesn't substitute `<<$pKey>>` the same way against Oracle data sources. 5+ sites in `apps/l2_flow_tracing/datasets.py` use this pattern. Need to test alternative approaches (concat done before binding; quoted-key path) and verify against deployed Oracle dashboard.
  - [ ] P.9f.c — `ORA-00923: FROM keyword not found where expected`. Some dataset SQL contains PG-only syntax Oracle rejects entirely. Common suspects: `LIMIT n` (Oracle wants `FETCH FIRST n ROWS ONLY`), `EXTRACT(EPOCH FROM ...)`, other PG-syntax that survived the P.4 dialect-aware sweep. Identify offending dataset(s) via QS console `cid` correlation to `boto3 describe_dataset`, port to dialect-aware emit.
