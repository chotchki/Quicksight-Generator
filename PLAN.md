...
- [ ] **X.2.h — Layer 2 e2e against the HTMX dialect.** Lift `tests/spike/test_html_layer2.py` pattern across the harness. Same fixture shape as the QS harness, same Layer-1 + Layer-2 assertions, different selectors:
  - **Status: X.2.h.1 (Executives) shipped.** `tests/e2e/test_html2_executives.py` spins the App2 server with the real Executives tree + a stub fetcher, drives Playwright (WebKit headless) against `/dashboards/exec`, asserts: (1) sheet tabs render with every analysis sheet, (2) Getting Started TextBoxes render via `_qs_richtext_to_html` (X.2.g.1.a polish — body > 200 chars), (3) visuals auto-load on DOMContentLoaded (X.2.g.1.a polish — KPI value appears without a Refresh click), (4) filter changes refetch (date_from in URL → fetcher call list), (5) filter form suppressed on text-box-only sheets, (6) filter form present on data-visual sheets. 6 tests pass under `QS_GEN_E2E=1`. Stub fetcher (not live PG) keeps the test fast + DB-free; live-PG variant lands as a CI matrix entry alongside `e2e-pg-browser` (same test shape, fetcher swapped for `make_tree_db_fetcher`). X.2.h.{2,3,4} for Investigation / L2FT / L1 land alongside X.2.g.{2,3,4}.
