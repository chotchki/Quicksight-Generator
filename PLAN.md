...
- [ ] **M.1 — L2 library foundation.** Lift the spike's working glue into proper library modules. Typed YAML loader matching SPEC; all SPEC validation rules with rejection tests; prefix-aware SQL emission; Current* projection. Acceptance: `from quicksight_gen.l2 import load_instance, validate, ...` works; spike's `slice.yaml` loads through it cleanly; pyright strict on `common/l2/`.
  ...
  - [ ] M.1.10 — **Per F7 (refactor):** promote `ScreenshotHarness` + supporting `browser_helpers` from `tests/e2e/` to a public location (`common/screenshot/` or `common/e2e/`). Production CLI (M.6, M.7, M.8) needs to invoke them; importing from `tests/` is a path hack the spike inherited. Existing test imports update to point at the new location. Per F8: also reshape the harness API to return `dict[Sheet, Path]` (keyed by Sheet ref, not sheet_id string) so handbook templates can look up screenshots by Sheet object instead of by sheet_id filename.
