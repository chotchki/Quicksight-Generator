...
- [ ] **U.7 — Provenance hash + verify subcommand.**
  - **Fingerprint scope (locked).** Hash the **base tables + external inputs**, NOT matviews — matviews are derived data; if their hash drifts from a recompute that's a *technical* signal (matviews need a refresh) but isn't authoritative for "what was this report bound to". The four base inputs:
    - **Transactions table** — high-water entry id (`MAX(transaction_id)`) + SHA256 over rows `WHERE id <= hwm` (canonical column order, deterministic serialization).
    - **Daily balances table** — high-water entry id (`MAX(daily_balance_id)`) + SHA256 over rows `WHERE id <= hwm`.
    - **L2 instance YAML** — SHA256 of the YAML file bytes (verbatim).
    - **quicksight-gen code** — git commit SHA of HEAD at generation time.
  - Composite fingerprint: `sha256(transactions_hwm || transactions_rows_sha || balances_hwm || balances_rows_sha || l2_yaml_sha || git_commit_sha)`. Short form (footer) = first 8 hex chars; long form (sign-off + cover provenance + verify command) = full 64-hex. Per-source breakdown is what the cover-page provenance table and the sign-off system block already expose (placeholders today).
  - Optional **matview-evidence sidecar**: separate from the authoritative fingerprint, optionally compute per-matview SHA256 and include in the cover provenance block as a second table titled "Derived view sanity (matview evidence)". A drift between these and a recomputed value flags a technical issue (stale matviews, schema change) rather than a data binding problem. **Decision deferred to U.7 implementation** — base-table fingerprint is the authoritative artifact either way.
  - Add `audit verify report.pdf -c config.yaml` — extracts embedded fingerprint from the PDF (or from the PDF metadata), recomputes against the connected DB at the embedded high-water marks + the L2 path on disk + git HEAD, compares. Exit 0 on match, 1 on drift, with a per-source diff so the operator sees WHICH input changed.
