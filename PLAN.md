...
- [ ] **L.0 — Tree spike (`spike/l-tree-account-network` branch).** Smallest possible port to validate the tree primitives produce JSON that the existing `models.py` accepts and that the deployed dashboard renders unchanged. Acceptance: byte-identical (or documented-semantic-equivalent) JSON for the Account Network sheet; deployed dashboard renders without regression. If fail: capture the failure mode and redesign the tree's API or emit interface before any L.1 work.
  - [ ] L.0.1 — Read the existing Account Network sheet code end-to-end: `apps/investigation/analysis.py::_build_account_network_sheet`, the relevant builders in `visuals.py` (Sankeys + table), the relevant filter groups in `filters.py` (anchor + min-amount + scoped CategoryFilters), and the constants in `constants.py`. Note every cross-reference: visual IDs into `SheetVisualScopingConfigurations`, parameter names into `ParameterControl`, dataset identifiers into the `LinkToDataSetColumn` for the dropdown.
  - [ ] L.0.2 — Sketch minimal L1 types as plain dataclasses in a scratch module (working name `common/_tree_spike.py`): `App`, `Dashboard`, `Sheet`, `Visual` (just enough subtypes for Sankey + table), `FilterGroup`, `ParameterDecl`, `FilterControl`, `ParameterControl`. No fancy validation yet — just structure + parent/child wiring + an `emit()` method per node that returns the corresponding `models.py` dataclass.
  - [ ] L.0.3 — Decide the cross-reference shape: does `FilterGroup` take a `list[Visual]` for its scope (object refs), or `list[VisualId]`? Sketch both and pick. Same for drill actions — destination is a `Sheet` ref or a `SheetId`?
  - [ ] L.0.4 — Tree-build the Account Network sheet end-to-end alongside the existing builder. Both produce a `SheetDefinition`; deep-diff the two via `to_aws_json()` output.
  - [ ] L.0.5 — Iterate: any diff that's a regression gets fixed in the spike's `_tree_spike.py`; any diff that's an intentional improvement gets a written justification. Repeat until diff is empty or fully documented.
  - [ ] L.0.6 — Capture findings as bullets at the end of this L.0 entry: what surprised, what API decisions emerged, what to lift into L.1. Spike branch either cherry-picks the validated primitives into L.1 or dies on the spike branch.
...
- [ ] **L.9 — Docs sweep + Python API doc generation.** New "Tree pattern" section in `CLAUDE.md` under Architecture Decisions covering L1 → L2 → L3 layering and the "Sasquatch lives only in L3" rule. README updates to count four apps and replace the constants-driven framing with the tree-driven one. New customization handbook walkthrough using the L.6 work as the worked example. **Plus mkdocstrings-driven Python API reference** (the `common/` tree is now meaty enough to need it). Acceptance: `mkdocs build --strict` clean; `pytest` green; all docs accurate against the post-L codebase; API reference rendered for every public class in `common/`.
  ...
  - [ ] L.9.10 — **Wire mkdocstrings into the mkdocs build.** Recommended tool: `mkdocstrings[python]` — auto-generates API reference pages from type hints + docstrings, integrates with the existing mkdocs-material site, no separate doc build step. Sub-steps:
    ...
    - Verify `mkdocs build --strict` still passes — strict mode will flag any broken docstring references, undocumented public symbols (if we configure that warning), or rendering errors.
    - Decide whether CI's `mkdocs build` step should fail on missing docstrings (warn-only initially; tighten to error after a docstring sweep).
