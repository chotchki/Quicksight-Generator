...
- [ ] **L.1 — Tree primitives in `common/`.** New module(s) under `common/` (working name `common/tree.py`; split into a `common/tree/` package if it grows past ~600 lines). Acceptance: primitives module shipped with full unit coverage; `to_aws_json()` over a tree-built `App` round-trips through deploy paths (no missing fields); the L.0 spike's Account Network port re-implemented against the full primitive set still emits byte-identical JSON.
  ...
  - [ ] L.1.6 — Typed `Filter` wrappers: `CategoryFilter` / `NumericRangeFilter` / `TimeRangeFilter` tree nodes. Same names as `models.py` — alias models on import inside `tree.py` (`from ... import CategoryFilter as ModelCategoryFilter`) so user code reads `from quicksight_gen.common.tree import CategoryFilter, NumericRangeFilter` without a `Node` suffix. Each takes object refs where the existing builders take strings: column = `(dataset, column_name)` pair (or a `Col` helper); `NumericRangeFilter`'s `minimum_parameter` / `maximum_parameter` take a `ParameterDeclLike` ref (the type checker catches "filter bound to undeclared parameter"). `FilterGroup.filters` switches from `list[models.Filter]` to a typed Protocol `FilterLike`.
  - [ ] L.1.7 — Typed `Dataset` tree nodes (App-level). Each dataset becomes a node owning its logical-id + ARN derivation; visuals/filters reference datasets by object ref instead of by string. `App.add_dataset(...)` registers; `Visual.dataset` / `Filter.dataset` fields take a `Dataset` ref. **Deployment side effect**: the tree gains a precise dependency graph for free — `App.dataset_dependencies()` walks the tree and returns the exact set of datasets each Sheet / Visual / FilterGroup / CalcField depends on. That graph drives selective deploy (only re-create datasets a downstream change touched) and matview REFRESH ordering (REFRESH only the matviews backing the changed deploy surface). The graph also catches "dataset declared but unused" and "dataset used but undeclared" classes at construction time.
  - [ ] L.1.8 — Typed `CalcField` tree nodes (Analysis-level). Each calc field becomes a node owning its name + dataset ref + expression; filters and field wells reference calc fields by object ref instead of by string column name. `Analysis.add_calc_field(...)` registers; the calc field's dataset ref participates in the L.1.7 dependency graph. Catches "filter on calc field that doesn't exist" + "calc field declared but never referenced" at construction time.
  - [ ] L.1.8.5 — Auto-IDs for internal-only IDs + tree-query helpers. **Mixed scheme:** URL-facing IDs (`SheetId`, `ParameterName`) and analyst-facing identifiers (`Dataset` identifier, `CalcField` name) stay explicit. Pure internal IDs (`VisualId`, `FilterGroupId`, layout element IDs, table/Sankey field-well IDs) get an `Optional[str]` shape — tree walker assigns at emit time using a position-indexed scheme (e.g. `v-kpi-0-an0`, `fg-1-an0`) when not specified. Result: in the common case, most IDs disappear from user code; the "ID appears once" principle becomes "the ID never appears at all unless the author needs to override." Plus tree-query helpers (`App.find_sheet(name=...)`, `Sheet.find_visual(title_contains=...)`, `Analysis.find_filter_group(...)`, etc.) so e2e tests look up nodes by title or structural position rather than hardcoded IDs — single source of truth, drift-proof. Validates against the L.1.15 re-port of Account Network: if a real sheet round-trips byte-identical with auto-IDs at structurally-equivalent positions, the API is good.
  - [ ] L.1.9 — Implement `FilterControl` + `ParameterControl` variants used today (dropdown / slider / date-range). Wire each to its source `FilterGroup` or `ParameterDecl` by reference (object ref into the L.1.4 / L.1.5 / L.1.6 nodes). Same naming convention as L.1.6: alias models on import; user code uses unsuffixed names.
  - [ ] L.1.10 — Implement `CrossSheetFilter` (the AR Today's Exceptions Check Type pattern) + drill actions (left-click = `DATA_POINT_CLICK`, right-click = `DATA_POINT_MENU`, navigate / set-parameter ops). Drill destination = `Sheet` reference, not `SheetId`. Drill action's parameter writes use `ParameterDeclLike` refs (object) and reuse `common/drill.py`'s `ColumnShape` for source-field shape validation.
  - [ ] L.1.10.5 — `TreeValidator` (`tests/e2e/tree_validator.py`). Typed walker that takes `(App, Page)` and asserts the deployed dashboard matches the source tree. Per-kind dispatch: each typed `Visual` / `Filter` / `Control` subtype has a corresponding `_validate_<kind>` method with kind-specific browser assertions (Sankey nodes render, Table columns match GroupBy + Values, KPI shows a number, dropdown populates from `LinkToDataSetColumn`, etc.). `validate_structure()` collapses the per-app structural e2e (`test_inv_dashboard_structure.py` etc.) — 100+ lines per app become one call. Persona-driven scenario tests stay focused on "did the right data show up" and skip the "does this primitive render" boilerplate. Lives in `tests/e2e/` — Playwright is a test-only dep, doesn't belong in `common/`.
  - [ ] L.1.10.6 — Kitchen-sink e2e test (`tests/e2e/_kitchen_app.py` + `test_tree_primitives.py`). Smallest persona-agnostic app that uses every typed tree primitive at least once: KPI / Table / BarChart / Sankey, every Filter variant (parameter-bound + static), calc fields, every control kind, mixed explicit + auto IDs, drill actions. Session-scoped deploy fixture; e2e test runs `TreeValidator.validate_structure()` plus per-primitive sanity checks. Catches QuickSight API drift on rarely-used primitives + missing-field bugs that pass unit emission but reject at deploy. New typed primitives we add later get one obligatory test by being added to the kitchen app.
  - [ ] L.1.10.7 — `ScreenshotHarness` (`tests/e2e/screenshot_harness.py` or `tools/screenshot_harness.py`). Walks a deployed App's tree and captures screenshots systematically. Three modes: `capture_all_sheets()` (one full-page screenshot per sheet), `capture_per_visual()` (scroll-into-view + element crop per visual, file-named by auto-ID), `capture_with_state(parameter_values=...)` (apply parameter values via URL hash or embedding SDK, then capture — for "before/after slider" handbook screenshots). Same Page-based infra as `TreeValidator` and the kitchen-sink test; different consumer. **Phase M payoff:** when whitelabel-V2 swaps personas, `ScreenshotHarness.capture_all_sheets()` regenerates the screenshot set against the new persona's data — automation that makes multi-persona shipping tractable. Today's manual screenshot capture doesn't scale to that.
  - [ ] L.1.11 — JSON emission round-trip test. Every tree node's `emit()` returns the corresponding `models.py` dataclass; tree-walk assembles the full `Analysis` + `Dashboard`. Test against a known-good output to catch any field-shape drift introduced during the typed-wrapper sub-steps.
  - [ ] L.1.12 — Construction-time validation hooks (formalize what's already accumulated): scope-on-same-sheet (L.1.5), drill source/destination shape compatibility (L.1.10 reuses K.2's `ColumnShape`), parameter-control type matches parameter declaration (L.1.9), dataset references resolve (L.1.7), calc-field references resolve (L.1.8). Audit + add the missing checks; document the rule set.
  - [ ] L.1.13 — Module docstrings on every public class + helper. These feed L.9.10 (mkdocstrings auto-generated API reference). Lint pass: every `class` and public `def` has a docstring; every typed field has its purpose noted.
  - [ ] L.1.14 — Unit tests sweep: confirm coverage for tree assembly, ID resolution, cross-reference resolution, validation rejections (bad scope, bad drill shape, undeclared parameter, missing dataset / calc field), JSON emission shape, builder ergonomics. Mirror `test_drill.py` shape from K.2.
  - [ ] L.1.15 — Re-port the L.0 spike's Account Network sheet against the full L.1 primitives — same byte-identity bar as the spike. This is the gate for L.2: if the typed primitives can reproduce the existing JSON exactly, the per-app port (L.2/L.3/L.4) is unblocked.
