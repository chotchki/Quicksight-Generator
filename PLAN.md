...
- [ ] **X.1.h.B — Doctest-style verification infrastructure.** Real-customer signal still active: the handbook over-promises (X.1.h.A fixed one instance; the existing walkthroughs reference column names like `transaction_id` that don't match the v6 schema's `id`). Want the Rust-doc equivalent: every fenced code block in handbooks gets executed (or at least cross-referenced) during CI. Candidate v0 (most pragmatic): a custom pytest collector that walks `docs/**/*.md`, extracts `quicksight-gen <cmd> <flags>` invocations from bash blocks, and asserts the command + flags exist in the live Click tree. That catches the "doc cites a removed CLI verb / flag" class without needing per-block fixtures. Layer in `pytest-markdown-docs` for Python blocks once we have fixtures sorted (most existing Python blocks reference `path/to/myorg.yaml` placeholder paths that need mocking). Defer SQL execution checks until there's a trivial in-memory DB target.
