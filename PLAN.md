...
- [ ] **Y.2.gate.b — Design the runner.** Single script (`./run_tests.sh up_to=<layer>`?  `tox`-style?  Make targets?) that composes the chain. Decisions:
    ...
    - **Variants axis (per layer)**: Each layer can be parameterized — same layer × N variants. The runner multiplexes variants and rolls failure status per (layer, variant) cell. Variants we know about today:
        ...
        - **Fuzz seed (property-testing axis — first-class)**: `tests/l2/fuzz.py::random_l2_yaml(seed)` produces a different valid L2 topology per seed. Today only one test consumes it at one seed/run. Audit §7.11: promote to a sampled axis across layers 2a/3a/3b/3c/3e/7 (~100 seeds/iteration, ~1000 nightly). Each Phase Y / X.2 sweep deepens the property coverage. Highest-value parallelism target — combinatorial coverage at constant wall-clock thanks to local Docker per-seed isolation.
    ...
    - **Per-run output isolation**: Every artifact lands under `runs/<run-id>/...` (run-id = `<utc-ts>-<short-sha>[-dirty]`). No more shared `run/out/`, `tests/e2e/screenshots/`, `tests/e2e/failures/`, `.coverage.*`, container logs. Last N=20 runs kept; auto-prune. `runs/` is gitignored.
    - **Timing capture + diff vs prior run**: Runner records per-(layer, variant, test) timings to `runs/<run-id>/timings.json`. On next invocation, reads the most-recent matching prior run and reports `step took X (was Y, ±Z%)`. Highlight (⚠) threshold: ±50% change (configurable). Same logical shape as hash-locked seed data — a stable artifact that flags drift loudly. Surfaces perf regressions / flakes / environmental drift before they cause failures. Optionally upload `timings.json` as a CI artifact for cross-CI-run drift visibility.
...
- [ ] **Y.2.gate.c — Implement the runner.** Probably a shell script + minimal Python wrapper. Reuse `./run_e2e.sh` infra where it fits. Concrete deliverables (per Y.2.gate.b spec):
    - Per-run `runs/<run-id>/` directory created at session start; all artifacts written there.
    - `runs/<run-id>/timings.json` recorded per (layer, variant, test).
    ...
    - Stop-on-first-failure across layers; xdist within layer.
    - `--up-to=<layer>`, `--variants=<set>`, `--only=<test-id>`, `--skip-cheap`, `--live-db` flags per Y.2.gate.b.
...
- [ ] **Y.2.gate.j — Parallelism (so the longer chain doesn't kill iteration speed).** Total wall-clock matters or developers will skip the chain to iterate. Strategy:
    ...
    - **Fuzz-seed parallelism is the headline parallelism win** (audit §7.11). 100-seed sample × per-seed local Docker × xdist completes in roughly the wall-clock of one sequential seed run. Each fuzz seed = a distinct synthetic L2 topology, so combinatorial coverage compounds with the dialect/L2-instance axis. Implement the seed pool as a deterministic sample (`Random(git_sha).sample(range(2**32), N)` or similar) so local + CI exercise the same set per commit and a regression is reproducible by pinning `QS_GEN_FUZZ_SEED=<N>`.
...
- [ ] **Y.2.gate.l — Ephemeral AWS infra (start/stop both ways).** Per audit §7.12. Today's persistent-Aurora pattern burns ~$45/mo Aurora idle + similar for RDS Oracle, even when no one is testing. Split:
    ...
    - **Decisions for user** (called out in §7.12): start-stop existing cluster vs. create-per-job in CI; granularity of `up aws` (combined or `up aws-pg` / `up aws-oracle`); auto-stop default; cost-visibility output format.
