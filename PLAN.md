...
- [ ] **Y.2.gate.b — Design the runner.** Spike (b.0) returned. Substrate locked: pytest-as-orchestrator + ~400 LOC Python wrapper at `quicksight_gen/_dev/runner.py`, surfaced via `./run_tests.sh` bash shim. Each design item below tracks against that substrate.
    ...
    - [x] **Y.2.gate.b.2 — Container-backed isolation for local-DB variants (LOCKED 2026-05-07).** Per-variant Docker containers via `testcontainers-python`. Under the locked substrate: each container is a session-scoped pytest fixture parametrized over dialect; the orchestrator dispatches per-variant pytest subprocesses, and within each subprocess the fixture brings up its container. Aurora live cell reserved for layer 4+. Audit §7.5.
    - [x] **Y.2.gate.b.3 — App2-as-early-gate chain placement (LOCKED 2026-05-07).** Layer 3.7 = App2 against local Docker. At merged 6/7: App2 + QS run in parallel with fast-fail. Under the locked substrate: orchestrator's `asyncio.gather` over `[target=app2, target=qs]` at layer 6/7 with `return_exceptions=False` (first failure cancels siblings). Audit §7.10. Strategic ownership in X.2.j.
    ...
    - [x] **Y.2.gate.b.8 — Skip-if-already-green policy (LOCKED).** Default re-run; opt-in `--skip-cheap`. Under the locked substrate: orchestrator's pre-dispatch check reads `.run_tests_cache/<sha>.json`; on `--skip-cheap`, layers 1-2 skip if green for the current SHA.
    ...
    - [ ] **Y.2.gate.b.10 — Dirty-state detection on deploy.** Lock the policy: `up_to=deploy` refuses to fire if `git status --porcelain` is non-empty (the `[-dirty]` run-id suffix is the warning signal; refusing the deploy is the next escalation). Override flag (`--allow-dirty-deploy`) for the rare "I know what I'm doing" case. Implementation lives in orchestrator's pre-flight (alongside dep probe).
    - [ ] **Y.2.gate.b.11 — Variant-to-layer enumeration (audit dependency).** Confirm Y.2.gate.a's audit doc layer table is complete. Pass: per-layer "applies-to" variant list flows into the orchestrator's dispatch table (one source of truth: parsed from the audit table or hard-coded mirror — decide during implementation).
    ...
    - [ ] **Y.2.gate.b.14 — Runner ergonomics for Claude-agent loops** (re-opened 2026-05-07; the bash-shim shape was the easy half; the harder half is making sure Claude can loop on the runner without thrashing the operator's `proceed?` permission prompt). The runner becomes the single Claude-Code Bash-tool boundary so an always-allow rule on `./run_tests.sh*` covers every test-loop invocation; **internal sub-tools (Docker, AWS, pytest) must run as subprocesses of the runner, never as separate Bash calls Claude reaches for directly**.
        ...
        - [ ] **Y.2.gate.b.14.3 — Destructive-op opt-in within the orchestrator** (so the Claude-Code always-allow doesn't yield surprise destruction). Orchestrator-side flag pattern: `down` / `sweep` / any DROP-shaped operation requires `--yes` (or `QS_GEN_RUNNER_YES=1` env). Without it, prints what it would do + exits non-zero. Idempotent reads (`up`, `status`, `up_to=<layer>`) need no flag. Mirrors `quicksight-gen json clean` requiring `--execute`.
...
- [ ] **Y.2.gate.c — Implement the runner.** Substrate: pytest + `quicksight_gen/_dev/runner.py` (~400 LOC budget) + `./run_tests.sh` bash shim (locked by `Y.2.gate.b.0` spike). Each sub-task carries the module / function / hook ownership.
    - [ ] **Y.2.gate.c.1 — Per-run directory creation + run-id format.** `runner.py::create_run_id()` → `<utc-ts>-<short-sha>[-dirty]`; orchestrator `mkdir -p runs/<run-id>/`; passes path to pytest subprocesses via `QS_GEN_RUN_DIR` env var.
    - [ ] **Y.2.gate.c.2 — Timings + hashes capture.** `tests/conftest.py::pytest_runtest_protocol` hook writes per-test timing to `$QS_GEN_RUN_DIR/timings/<variant>.json`. Session-scope fixture writes `$QS_GEN_RUN_DIR/hashes/<variant>.json`. Orchestrator `runner.py::collect_run_outputs(run_id)` merges into top-level `timings.json` + `hashes.json`.
    - [ ] **Y.2.gate.c.3 — Drift-diff report.** `runner.py::diff_against_prior(run_id)` finds most-recent prior run for current SHA (else most-recent overall); reports `step took X (was Y, ±Z%)` with ⚠ at ±50%. Single-source-of-drift covers timings + hashes + row-counts in the same loop.
    - [ ] **Y.2.gate.c.4 — `runs/` gitignored + auto-prune past N=20.** Add `runs/` to `.gitignore`. `runner.py::prune_old_runs()` runs at session end; keeps last 20 by mtime.
    - [ ] **Y.2.gate.c.5 — Stop-on-first-failure across layers.** Orchestrator's main loop: `for layer in chain: results = await dispatch_layer(layer); if any_failed(results): break`. Per `Y.2.gate.b.7`, per-variant within a layer rolls up (siblings finish even if one fails); App2 + QS at 6/7 are the fast-fail exception.
    - [ ] **Y.2.gate.c.6 — Within-layer parallelism.** Cross-variant: `await asyncio.gather(*[run_pytest(layer, variant) for variant in variants])`. Within-variant: pytest-xdist via `--parallel=N` flag passthrough (existing `./run_e2e.sh` knob carries over). App2 + QS at 6/7: same `asyncio.gather` over `[target=app2, target=qs]` with `return_exceptions=False`.
    - [ ] **Y.2.gate.c.7 — Flag plumbing.** `runner.py::main()` argparse: `--up-to=<layer>`, `--variants=<set>`, `--only=<test-id>`, `--skip-cheap`, `--fuzz-seeds=N`, `--keep-on-failure` (per `Y.2.gate.f.5`), `--trace-all` (per `Y.2.gate.b.4`), `--allow-dirty-deploy` (per `Y.2.gate.b.10`). All translate to pytest CLI flags or env vars passed to subprocesses.
    - [ ] **Y.2.gate.c.8 — Dependency probe (no state file, LOCKED §7.12).** `runner.py::probe_dependencies(layer)` runs before dispatch: `aws sts get-caller-identity`, `psycopg.connect`, `docker ps` per the layer's needs. Failures emit clear messages (`"layer 4 needs AWS DB but cluster is stopped — run './run_tests.sh up' first or invoke './run_tests.sh up_to=app2-e2e' to skip"`).
    - [ ] **Y.2.gate.c.9 — General-sweep cleanup subcommand.** `./run_tests.sh sweep` → `runner.py::sweep_orphans()` cleans orphan resources tagged `ManagedBy:quicksight-gen`. Replaces `scripts/sweep_harness_orphans.py` (per `Y.2.gate.f.8`) + cleans up after `--keep-on-failure` sessions.
    - [ ] **Y.2.gate.c.10 — Top-queries auto-capture (per `Y.2.gate.f.4`).** `tests/conftest.py` session-scope teardown fixture (per dialect) dumps top queries to `$QS_GEN_RUN_DIR/db/<dialect>/top-queries.md`. Replaces `scripts/dump_top_queries.py`.
    - [ ] **Y.2.gate.c.11 — Browser trace capture.** `tests/e2e/conftest.py` Playwright fixture writes JS console + network traces / HAR / video / screenshots under `$QS_GEN_RUN_DIR/browser/<test-id>/` (failure-only default per the fixture's `pytest_runtest_makereport` outcome check; `--trace-all` opt-in via fixture param).
    - [ ] **Y.2.gate.c.12 — L2 YAML always copied.** `tests/conftest.py` session fixture: every L2 instance the run touches gets copied to `$QS_GEN_RUN_DIR/l2/<instance-or-seed>.yaml` — fuzzed AND static. Hooked via the existing L2-loader.
    - [ ] **Y.2.gate.c.13 — `--seed-density=N` tunable generator parameter** (audit §7.13 LOCKED 2026-05-07; lands with the hash-lock move). Drift detection moves from frozen global hash to run-pair diff via `Y.2.gate.c.3`.
        - [ ] **c.13.1 — Plumb `--seed-density=N` through `common/l2/seed.py::emit_full_seed`** as a parameter; threads through CLI + runner.
        - [ ] **c.13.2 — Drop the global SHA256 lock in `tests/data/test_l2_baseline_seed.py`** in favor of the per-run hash recorded in `runs/<run-id>/hashes.json`.
        - [ ] **c.13.3 — Regression guard test:** previous default density produces same byte-output as before the change (one-shot fixture-data identity test against a checked-in golden).
        - [ ] **c.13.4 — Document the tunable in CLAUDE.md** alongside the `runs/<run-id>/` pattern.
...
- [ ] **Y.2.gate.f — "Validate, then delete-or-fold" sweep over scripts/ + harness layer.** Locked principle (audit §7.1, 2026-05-07): every standalone script gets a validation step — if duplicated by existing tests, delete; if not, fold into the runner (as a per-run output, runner flag, or runner subcommand). No scripts hanging around as "I might need this someday." Same shape applies to the harness layer (§7.7 LOCKED 2026-05-07 — drop layer 8 as a distinct lane).
    - [ ] **Y.2.gate.f.1 — `tests/integration/verify_dataset_sql.py` → delete.** Duplicated by `tests/e2e/test_dataset_sql_smoke.py`. Drop the CLI variant + the CI step that calls it.
    - [ ] **Y.2.gate.f.2 — `tests/integration/verify_demo_apply.py` → fold into pytest.** Convert to `tests/e2e/test_demo_apply_row_counts.py`. CI calls the pytest version. Delete the CLI.
    - [ ] **Y.2.gate.f.3 — `scripts/bake_sample_output.py` → delete (verify first).** User: "unneeded". Currently used by `release.yml::bake` to produce `out-sample.zip` as a release artifact. Verify whether the published wheel + docs make the sample bundle redundant; if so, drop both the script and the release step.
    ...
    - [ ] **Y.2.gate.f.6 — `scripts/m2_6_verify.py` → delete (verify first).** User: "should be covered by existing tests". Pytest sweep needs to confirm coverage before drop.
    - [ ] **Y.2.gate.f.7 — `scripts/qs_substitution_probe.py` → fold into test output if not duplicated.** If the probe surfaces something not already in `verify_dataset_sql.py` / `test_dataset_sql_smoke.py`, fold the probe's output into per-run capture (e.g., post-deploy diagnostic dump). Otherwise delete.
    ...
    - [ ] **Y.2.gate.f.10 — P.9f.d alignment with Oracle e2e work.** Sole remaining `P.9f.*` task is `P.9f.d — Re-run 4-cell e2e matrix`. Two-step plan (LOCKED 2026-05-07):
        - [ ] **f.10.1 — Research step.** Do we already have what we need? With `Y.2.gate.f.9`'s harness collapse + the merged 6/7 layer running both targets (`qs | app2`) × both dialects (`pg | oracle`) under §7.4's nightly-cron Oracle browser cell, the runner naturally produces "all-cells green" output on every full invocation. Confirm: (a) the runner's CI parity (`Y.2.gate.k`) covers all 4 cells (PG-API / PG-browser / Oracle-API / Oracle-browser) and (b) the per-run capture surfaces enough signal to triage failures without bespoke re-run tooling. Read the audit §7.4 + §7.7 + Y.2.gate.k to confirm coverage.
        - [ ] **f.10.2 — Decide based on research.** If (a) + (b) hold: tick `P.9f.d` complete (the runner subsumes the manual re-run) and remove it from PLAN.md. If we surface a coverage gap: expand Y.2.gate.k or add a dedicated sub-task to fill it. If we genuinely can't tell: delete `P.9f.d` and accept the risk — the audit's framing is that the new orchestration was always going to obsolete this class of task.
- [ ] **Y.2.gate.h — Credential auto-discovery.** Operators / Claude should never have to look up or pass credentials by hand. Acceptance: with a fresh shell + AWS SSO logged in + cfg in place, every layer's invocation works without any env-var exports.
    - [ ] **Y.2.gate.h.1 — `QS_E2E_USER_ARN` derived from AWS identity.** `aws sts get-caller-identity` → IAM principal → `aws quicksight list-users` filter by `PrincipalId` → pick matching ARN. Drops the memory entry that just remembers the value.
...
- [ ] **Y.2.gate.i — AWS auth refresh.** Constant `aws sso login` friction is automated away. Claude should never have to ask the user to "re-login" mid-flow — the runner handles it.
    - [ ] **Y.2.gate.i.1 — Design.** Decide pluggable-login config shape so operators on `aws-vault` / role-assumption / static keys can wire their flow (`run/auth.yaml` or similar). Pre-flight check: `aws sts get-caller-identity`. **On `ExpiredTokenException` / `UnrecognizedClientException`: detect-and-surface, do NOT auto-invoke** (per `Y.2.gate.b.14.4`'s refusal pattern). Print `"creds expired — type '! aws sso login' yourself, then re-invoke"` and exit code 2 = needs-operator. Earlier draft of this task said "auto-refresh" — that's wrong because `aws sso login` opens a browser and Claude would hang. The pluggable layer is for *which command the operator runs*, not for the orchestrator to invoke.
    - [ ] **Y.2.gate.i.2 — Instructions.** Document the operator config schema for `run/auth.yaml` + the precedence order; document how to wire alternative auth flows (`aws-vault`, role-assumption, static keys). Lands in CLAUDE.md test-sequencing section + the runner's `--help` output.
    - [ ] **Y.2.gate.i.3 — Build.** Implement runner pre-flight + pluggable login dispatch + retry on the configured layer.
    - [ ] **Y.2.gate.i.4 — Validate.** Acceptance: starting a layer that needs AWS with expired creds → (1) a single login prompt fires, (2) the layer runs, (3) no manual rerun needed. Test with the project's actual AWS SSO flow + at least one alternative (e.g. mock the failure path).
- [ ] **Y.2.gate.j — Parallelism (so the longer chain doesn't kill iteration speed).** Total wall-clock matters or developers will skip the chain to iterate.
    ...
    - [ ] **Y.2.gate.j.5 — Container reuse policy.** PG containers cheap (~5s start) — recreate per session OK. Oracle 19c heavy (~30-60s, ~5GB) — name-cache across runs in the same session (`quicksight-test-oracle`), recreate only on schema drift. SQLite in-process; no container.
    - [ ] **Y.2.gate.j.6 — Within-layer-within-variant = pytest-xdist.** Existing `--parallel` knob from `./run_e2e.sh` carries over. Layer 3a's 37 datasets + layer 5/6's per-app suites all benefit.
    - [ ] **Y.2.gate.j.7 — Targeted-iteration mode (`--only=<test-id>`).** Runs the full chain but narrows within-layer-within-variant to one test id. Use case: tweaking one Investigation drill, validate the chain through layer 6 with only `test_inv_drilldown.py::test_X`.
    - [ ] **Y.2.gate.j.8 — Skip-cheap mode (`--skip-cheap`).** Skips layers 1-2 if green on the same git SHA earlier in the session. Off by default; opt-in for tight loops. Cache lives in `.run_tests_cache/<sha>.json` so a `git commit` invalidates.
...
- [ ] **Y.2.gate.m — `Y.2.c+ unblocked` gate.** Once a-l are done, resume the Y.2 sweep. (Renamed from `Y.2.gate.g` 2026-05-07 so alphabetical order = positional order; this is the terminal gate, not a mid-list item.)
