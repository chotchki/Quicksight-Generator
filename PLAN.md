...
- [ ] **K.4.1 — Reorg under `apps/` namespace + docs/training cleanup.** Mechanical re-org earning the 4.0.0 bump. Move `src/quicksight_gen/payment_recon/` → `apps/payment_recon/` and `account_recon/` → `apps/account_recon/`; update every import across the codebase + tests; update `pyproject.toml` package discovery if needed. Docs/training tree consolidation: today there are two parallel handbook trees (`docs/handbook/*.md` for the four app/etl/customization handbooks shipped with mkdocs; `training/handbook/{concepts,scenarios,for-accounting,for-customer-service,for-developers,for-product-owner}/` for the audience-organized training material) plus Python utilities (`training/distribute.py`, `training/publish.py`) mixed into the training directory. Decision needed early: (a) merge training handbook into `docs/`; or (b) keep training material distinct from operator docs and move utilities to `tools/` so `training/` is purely content. After this sub-phase: one clear answer to "where do shipped docs live?" and "where does Python live?" — tested by a fresh `pip install -e .` + `quicksight-gen --version` + `mkdocs serve` + a green test suite.
