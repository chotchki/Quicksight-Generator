...
- **K.2 — Cross-sheet drill parameter hygiene + typed helpers.** Released in **v3.6.0**. All six AR drill `FilterGroup`s switched from parameter-bound `CategoryFilter`s (which silently match the empty-string sentinel and suppress every row) to a calc-field PASS shape. New `common/drill.py` introduces `ColumnShape` + `DrillParam` + `DrillSourceField` + `cross_sheet_drill()` that refuse a wiring whose source-field shape can't assign to the destination param's expected shape — fails at the buggy line, not in a downstream output-walking test. All 13 drill sites (7 PR + 6 AR) migrated; the 4 AR drills into Transactions auto-reset every PASS-filtered param the caller doesn't write. `ar_unified_exceptions` became a `MATERIALIZED VIEW` so Today's Exceptions renders under Direct Query. See `RELEASE_NOTES.md`.
- **K.2 follow-up still owed** (rolled into K.3 below since both touch schema docs): document the `ar_unified_exceptions` matview refresh contract in `docs/Schema_v3.md` ("Materialized views" section listing the view, the `REFRESH MATERIALIZED VIEW` requirement after each ETL load, and the timing semantics for the `days_outstanding` / `aging_bucket` columns). `demo apply` already issues the refresh; production ETL must do the same.
