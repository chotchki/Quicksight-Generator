...
- [ ] **Y.1.k — Cascade dropdown probe.** REOPENED: I assumed L2FT's metadata cascade (`build_l2ft_postings_dataset` — `<<$pKey>>` + `<<$pValues>>` + `MappedDataSetParameters`) was working in production because X.1.g shipped + browser e2e was green. **User correction (during Y.1 spike): L2FT metadata filtering DOES NOT actually work right today in production. The X.1.g browser e2e was checking dropdown population / selection, not the actual filter applying to the underlying data.** Working hypothesis: a "layer filter violation" between the dataset parameter, the MappedDataSetParameter bridge, and the per-visual filter scope. May explain why pure-cascade-driven filtering hasn't held up. Action items (revisit before Y.2's L2FT sweep): (1) reproduce the L2FT cascade failure with concrete example (which dropdown? what user action? what does QS actually show vs expected?). (2) trace the layer filter chain through the QS deployment to identify the violation. (3) decide whether the fix lands inside Phase Y (we're already touching this surface) or as a precursor task. (4) until resolved, treat cascade behavior under SQL pushdown as **unverified**, not proven.
...
#### Y.1.n — Cross-sheet / cross-dashboard URL-param drill re-evaluation

**Premise (contingent on Y.1.m landing).** If Y.1.m proves URL params actually drive analysis parameter values (not just control widget display), the K.4.7 cross-app drill capability comes back into reach. Currently the codebase carries a pile of workarounds we built when URL → param was broken:

- **K.4.7 dropped drills**: AR account row → Inv Account Network anchor; L1 violation → Inv Recipient Fanout; Inv counterparty → AR account detail. Code was removed; task list shows them dropped, the memory `project_qs_url_parameter_no_control_sync.md` documents the bypass.
- **Sentinel "all" branches in dataset SQL**: every cascading dataset has `<<$pKey>> = '__SENTINEL__'` as the first OR branch so the visual renders SOMETHING when params aren't filled — the sentinel was the assumed fallback because URL params were unreliable.
- **Companion / placeholder datasets**: Y.1.b.companion exists because we couldn't trust the parameter would propagate cleanly to a per-visual scope; the workaround was a second dataset without the parameter at all.
- **Sheet-description honesty notes**: Account Network sheet description carries "trust the chart, not the control" because the control text-state lagged the data state.

If URL → param works post-Y.1.m, several of those become unnecessary. Specifically, cross-app drills become a viable connective tissue across the 4 apps (L1 ↔ L2FT ↔ Investigation ↔ Executives) — that's what makes "huge win" the right framing.

- [ ] **Y.1.n.1 — Re-test L2FT cascade post-Y.1.m deploy.** Same URL stamp from Y.1.m.1 (`?pL2ftMetaKey=customer_id&pL2ftMetaValue=cust-0001-snb`); confirm Transactions table narrows to ~49 rows. Gate for everything below.
- [ ] **Y.1.n.2 — Cross-sheet within Investigation app.** Test URL-stamped sheet entry: e.g., navigate to Account Network sheet with `?pInvANetworkAnchor=<account>` pre-stamped. Confirm anchor dropdown populates AND the directional Sankeys narrow. If yes → cross-sheet binding within one analysis works without intermediate state.
- [ ] **Y.1.n.3 — Cross-dashboard (across apps).** Test URL-stamped cross-app entry: e.g., L1 Limit Breach sheet → Investigation Recipient Fanout with `?pInvFanoutThreshold=N` stamped. The K.4.7 drill we dropped lands on the Inv dashboard URL with the param query string; verify the Fanout sheet narrows.
- [ ] **Y.1.n.4 — Re-enable dropped K.4.7 drills.** If Y.1.n.2 + Y.1.n.3 both pass: re-introduce the three dropped cross-app drills via `CustomActionURLOperation`. Likely scope: AR violation row → Inv anchor handoff (the original use case), Inv counterparty → AR account drill, L1 limit-breach row → Inv Fanout pivot. Each drill is one builder-helper call; tests assert the URL shape.
- [ ] **Y.1.n.5 — Audit sentinel-default workarounds.** Walk the dataset SQL strings in `apps/*/datasets.py`; flag every `<<$pParam>> = '__SENTINEL__'` OR-branch and decide whether the param will always be URL-supplied OR is genuinely optional (analyst landing on a sheet with no upstream context). Keep sentinels only where the optional-load case is real.
- [ ] **Y.1.n.6 — Audit companion datasets / placeholder hacks.** `DS_INV_VOLUME_ANOMALIES_DISTRIBUTION` is a clean per-visual-scope companion (Y.1.b.companion). Look for similar workarounds we built when binding was broken — e.g., the meta-values-dataset narrow-accounts pattern (K.4.8k). Verify each remains necessary post-Y.1.m. Likely we keep the companion-dataset pattern (it serves SELECTED_VISUALS scope cleanly) but drop sheet-description honesty notes if the control + data now agree.
- [ ] **Y.1.n.7 — Update memory + quirks log.** Rewrite `project_qs_url_parameter_no_control_sync.md` with the refined finding: URL params populate BOTH control widgets and analysis parameters provided the parameter is single-valued OR the URL passes a properly-shaped multi-value list. Append the L2FT-cascade-via-text-field defect to `docs/reference/quicksight-quirks.md`.
- [ ] **Y.1.n.8 — Cross-link handbook section.** New page or section in `docs/handbook/` documenting the cross-sheet / cross-dashboard URL-stamping pattern. Three concrete worked examples (cross-sheet, cross-dashboard within app, cross-app). Builds the surface area for analysts to use the connective tissue.
