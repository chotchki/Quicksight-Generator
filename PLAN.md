...
- [ ] **X.4.c.5** — Coverage-tint overlay: `coverage_for(connection, prefix, l2_instance)` data fetcher (binary presence per L2 primitive, row-count on hover).
  ...
  - [ ] **X.4.c.5.f** — Tests: unit for `coverage_for` against SQLite + a hand-built `<prefix>_transactions` ✅ (7 tests in `test_l2_coverage.py`); route + chrome integration via Starlette `TestClient` ✅ (4 tests in `test_studio_diagram_coverage_route.py` — graceful degrade without pool, JSON shape with pool, chrome toggle present with pool, loud-fail on missing dialect). Browser-driven Playwright e2e (toggle the checkbox, assert `data-presence` on rendered SVG) deferred until we have a Studio in-process driver — the unit tier covers the contract and the JS handler is renderer-shape-locked to the JSON the route returns.
